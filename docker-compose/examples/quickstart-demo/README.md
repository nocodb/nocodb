...
Smallest possible production-shaped configuration. Bundled PostgreSQL, bundled Redis, NocoDB exposed on port 8080. No reverse proxy, no SSL. Self-contained — every value lives in `docker-compose.yml`, so `docker compose up -d` works out of the box.

- **PostgreSQL**: Bundled (data persisted to `./postgres`)
- **Redis**: Bundled (data persisted to `./redis`)
