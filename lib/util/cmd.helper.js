...
program
  ...
  .option("-h, --host <n>", "hostname of database / localhost by default", process.env.DATABASE_HOST)
  .option("-u, --user <n>", "username of database / root by default", process.env.DATABASE_USER)
  .option("-p, --password <n>", "password of database / empty by default", process.env.DATABASE_PASSWORD)
  .option("-d, --database <n>", "database schema name", process.env.DATABASE_NAME)
  .option(
    "-r, --ipAddress <n>",
    "IP interface of your server / localhost by default", 
    process.env.API_HOST
  )
  .option("-n, --portNumber <n>", "port number for app / 3000 by default", , process.env.API_PORT)
  ...
  .option("-x, --expires <s>", "REST API token expiration, default '30d'", process.env.TOKEN_EXPIRES)
  .option("-D, --DEV", "DEV mode, disables JWT token authorization")
  .option(
    "-c, --useCpuCores <n>",
...
exports.handle = program => {
  ...
  program.expires = program.expires || "30d";
  program.DEV = program.DEV  || false;
