...
program
  ...
  .option('-b, --bitToBool', 'bit to boolean conversion / false by default')
...
exports.handle = program => {
  ...
  program.bitToBool = program.bitToBool || false;

  // Bit fields interpreted as boolean need conversion from buffer / bool
  if(program.bitToBool){
    program.typeCast =  function (field, next) {
      // handle only BIT(1)
