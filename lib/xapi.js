...
class Xapi {
  constructor(args, mysqlPool, app) {
    ...
    this.app = app;
    this.ctrls = [];
    this.protected = (req, res, next)=>{ next() }; // passport, requires jwt authorized token
  ...
  init(cbk) {
    this.mysql.init((err, results) => {
      ...
      this.protected = this.initPassportMiddleware();

      let stat = this.setupRoutes();
      this.app.use(this.errorMiddleware.bind(this));


      //  set owner AFTER authentication
      this.mysql.setOwner("rjsa94b7ytuc")
      console.log("1>> set owner, oid=", this.mysql.oid)
  ...
  initPassportMiddleware(){
    ...
    async function _login(req, res) {
      // console.log(">>> _login, query=", req.query)
        ...
        switch (routes[i]["routeType"]) {
          case "list":
            this.app
              ...
              .get(this.protected, this.asyncMiddleware(resourceCtrl.list.bind(resourceCtrl)));
            break;
