...
class Xapi {
  ...
  constructor(args, mysqlPool, app) {
    ...
    this.swagger.title(JSON.stringify(this.config.database));
    /**************** START : multer ****************/
  ...
  setupRoutes() {
    ...
    for (var j = 0; j < resources.length; ++j) {
      this.swagger.addDefinition(resources[j]);
      ...
      for (var i = 0; i < routes.length; ++i) {
        ...
        switch (routes[i]['routeType']) {
          ...
          case 'list':
            ...
            this.swagger.addGetAll(routes, i, resourceCtrl, j, resources[j]);
            break;

          case 'findOne':
            this.app.route(routes[i]['routeUrl'])
              .get(this.asyncMiddleware(resourceCtrl.findOne.bind(resourceCtrl)));
            this.swagger.addGetSingle(routes, i, resourceCtrl, j, resources[j]);
            break;
          ...
          case 'create':
            if (!this.config.readOnly)
              this.app.route(routes[i]['routeUrl'])
                ...
                this.swagger.addPostSingle(routes, i, resourceCtrl, j, resources[j]);
            break;
          ...
          case 'update':
            if (!this.config.readOnly){
              this.app.route(routes[i]['routeUrl'])
                ...
                this.swagger.addPutSingle(routes, i, resourceCtrl, j, resources[j]);
            } else {
              stat.apis--;
            }
            break;
    ...
    /**************** START : Swagger Docs ****************/
    this.app.get('/swagger.json', this.asyncMiddleware(this.swaggerJson.bind(this)));
    stat.apis += 1;
    /**************** END : Swagger Docs ****************/
  ...
  /**************** END : health and version ****************/
  /**************** START : Swagger docs ****************/
  async swaggerJson(req, res) {
    res.json(this.swagger.getJson());
  }
