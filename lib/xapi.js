...
class Xapi {
  ...
  init(cbk) {
    this.mysql.init((err, results) => {
      ...
      if (DEBUG){
        // do NOT authenticate token, accept oid=* as wildcard
        const pass_authorization_callback = (req, res, next)=>{ 
          if (DEBUG){
            if (req.query.oid==="*") {
              this.mysql.setOwner(null);  // owner access wildcard for DEBUG=true only
              req.query.oid = null;
            } else {
              this.mysql.setOwner('unauthorized');
            }
            next();
          }
          else res.status(401).json({ msg: 'Unauthorized. set debug=true for dev' });
        };
        const login_route_pass_authorization_callback = (req,res)=>{
          res.status(200).json({
            msg: "ok",
        ...
        this.authenticateToken_callback = pass_authorization_callback;
        this.app
        .route(this.config.apiPrefix + "login")
        .get(this.asyncMiddleware(login_route_pass_authorization_callback))
        .post(this.asyncMiddleware(login_route_pass_authorization_callback));
  ...
  initPassportMiddleware() {
    ...
    // return express callbacks
    const authorization_callback = passport.authenticate('jwt', { session: false })
    const login_callback = async (req, res) => {
      ...
      if (oid && username) {
        ...
        return res.status(200).send({ msg: 'ok', token: token });
      } 

      return res.status(500).send("unknown error on login");
    }

    //  ???: why do I have to set route here? problem has something to do with async/await
    this.app
    .route(this.config.apiPrefix + "login")
    .post(this.asyncMiddleware( login_callback.bind(this) ));
