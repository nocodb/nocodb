...
class Xapi {
  ...
  setupRoutes() {
    ...
    // add route for generate token
    if(this.config.jwtSecret) {
      this.app
        .route(this.config.apiPrefix + "token")
        .post(this.asyncMiddleware(this.getToken.bind(this)));
    ...
    if(this.config.jwtSecret) {
      this.app.use(this.jwtToken.bind(this));
    }
  ...
  async getToken(req, res) {
    ...
    // expires 1 day
    const token = jwt.sign(payload, this.config.jwtSecret, { expiresIn: '1d' });
