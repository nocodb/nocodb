...
class xctrl {
  ...
  async changePassword(req) {
    ...
    const update_passwd_query = `UPDATE ${user.tableName} SET ${user.password}=? WHERE ${user.key}=? AND ${user.username}=?`;
  ...
  async getUser(req) {
    ...
    const user = Object.assign({},this.mysql.userSchema);
    const md5_password_query = `SELECT ${user.key}, ${user.username} FROM ${user.tableName} WHERE ${user.username}=? AND ${user.password}=? LIMIT 1`;
    const bcrypt_password_query = `SELECT ${user.key}, ${user.username}, ${user.password} FROM ${user.tableName} WHERE ${user.username}=? LIMIT 1`;
    ...
    .then( results=>{
      ...
      // console.log( `md5>>> password=${password}`.green, resp )
