...
class xctrl {
  ...
  async getUser(req, res){
    ...
    const query = `SELECT ${user.key}, ${user.username} FROM ${user.tableName} WHERE ${user.username}=? AND ${user.password}=? LIMIT 1`;
    ...
    if (username && password) {
      ...
      // console.log(">>> getUser(): login=", query, params);
      var results = await this.mysql.exec(query, params);
      let resp = results.pop()
      ...
      // console.log(">>> getUser()", resp.member_id, resp.username, username)
