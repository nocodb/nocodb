...
class Xsql {
  ...
  prepareListQuery(req, res, queryParamsObj, listType = 0, oid = null) {
    ...
    if (listType === 1) {
      ...
      this._restrict2OwnerId(req, req.app.locals._tableName, queryParamsObj, oid)
  ...
  _restrict2OwnerId(req, tableName, queryParamsObj, oid=null){
    // add members hasMany relationship and limit to members.member_id=oid
    // NOTE: run BEFORE this.getWhereClause()
    oid = oid || this.oid
    if (!!oid) {

      // console.log(">>> add Owner privilege, oid=", oid, tableName)
      // console.log(">>> req.query._join=", req.query._join)
      // const check = this.metaDb.tables[tableName]["columns"]
      // console.log(check)

      const columns = this.metaDb.tables[tableName]["columns"]
      if (columns && columns.find(o=>o['column_name']=='member_id')) {        
        // join with member_id and add join
        let joinTables = req.query._join && req.query._join.split(",");
        if (!joinTables || !~joinTables.indexOf('m.members')){
          this._joinOwnerId(req, tableName, queryParamsObj)
        }
      }
      this.getWhereClause(
        `(m.member_id,eq,${oid})`,
      ...
      if (!!req.query._where){
        // prefix alias to any column names in the where clause
  ...
  _joinOwnerId(req, fkTable, queryParamsObj){
    // from prepareJoinQuery()
    // console.log("fields=", fields) # prefix fields with alias 'fk'
    // queryParamsObj.query = "[...] as fk join ?? as ??  on (??=??)"
    // queryParamsObj.params = [ '...', 'fk', 'members', 'm' , `fk.member_id`, 'm.member_id']

    const fields = req.query._fields && req.query._fields.split(",");
    if (fields) {
      queryParamsObj.params = queryParamsObj.params.map((v,i,l)=>{
        return i+1<l.length ? `fk.${v}` : v
      });
    }
    queryParamsObj.query += "as ?? join ?? as ??  on (??=??) "
    queryParamsObj.params = queryParamsObj.params.concat( ['fk', 'members', 'm', `fk.member_id`, 'm.member_id'] )
  }
