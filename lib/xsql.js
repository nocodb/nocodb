...
class Xsql {
  constructor(sqlConfig, pool) {
    ...
    // limit gets to `member_id=${this.oid}`
    this.oid = 'unauthorized';  // default to unauthorized when DEBUG=true, add oid to queryparams
    this.userSchema = {
      tableName: 'members',
      alias: 'm',
      key: 'member_id',
      username: 'username',
      password: 'password',
      password_https: 'password_https',   // upgrading users to passwords protected by SLL/TLS schema
    }
  ...
  getLimitClause(reqParams) {
    //defaults
    reqParams._index = 0;
    reqParams._len = PER_PAGE_DEFAULT;

    if ("_size" in reqParams) {
      if (parseInt(reqParams._size) > 0 && parseInt(reqParams._size) <= PER_PAGE_MAX) {
        reqParams._len = parseInt(reqParams._size);
      } else if (parseInt(reqParams._size) > PER_PAGE_MAX) {
        reqParams._len = PER_PAGE_MAX;
      }
    }
  ...
  getWhereClause(queryparams, tableName, queryParamsObj) {
    if (queryparams) {
      let whereClauseObj = whereHelp.getConditionClause(queryparams);
      // ignore param: appendToWhere
      let appendToWhere = queryParamsObj.query.toLowerCase().includes('where') ? " and " : " where ";
   ...
   */
  prepareListQuery(req, res, queryParamsObj, listType = 0, oid = null) {
    ...
    if (listType === 1) {
      ...
      // console.log(`\n>>> ${req.route.path}`, queryParamsObj);

      this._ownerAccess_where(req, req.app.locals._tableName, queryParamsObj, oid)
      // console.log(`1>> ${req.route.path}`, queryParamsObj);
      /**************** add where foreign key ****************/
      let fkWhereClause = this.getForeignKeyWhereClause(
      ...
      // console.log(`2>> ${req.route.path}`, fkWhereClause, req.query._where);

      oid = oid || this.oid
      if (!!oid) {
        // alias the ForeignKeyWhereClause
        const parentAlias = queryParamsObj.params.reduce( (res,v)=>{
          if (res==="next") res=v; // done
          else if (v===req.app.locals._parentTable) res = "next"
          return res
        }, null );
        fkWhereClause = `${parentAlias}.${fkWhereClause}`;
        // console.log(`3>> ${req.route.path}`, fkWhereClause, req.query._where);
      } 

      const appendToWhere = queryParamsObj.query.toLowerCase().includes('where') ? " and " : " where ";
      queryParamsObj.query += `${appendToWhere} (${fkWhereClause}) `;
      ...
      // console.log(`4>> ${req.route.path}`, whereClause, queryParamsObj);
    ...
    this._getGrpByHavingOrderBy(req, req.app.locals._tableName, queryParamsObj, listType);
    
    // console.log(`\n>>> ${req.route.path}`, queryParamsObj.query, queryParamsObj.params);
  }


  _ownerAccess_where(req, tableName, queryParamsObj, oid=null){
    // add tableName belongsTo userSchema.tableName relationship and limit to `${userSchema.alias}.${userSchema.key}`=oid
    const userSchema = this.userSchema
    const belongsToSchema = Object.assign({}, this.belongsToSchema) // clone
    
    // NOTE: run BEFORE this.getWhereClause()
    oid = oid || this.oid
    if (!!oid) {

      const columns = this.metaDb.tables[tableName]["columns"]
      const joinTables = req.query._join && req.query._join.split(",");
      const isUserSchemaTable = !joinTables && tableName==userSchema.tableName;
      const isAlreadyJoined = !!joinTables && joinTables.find( v=>v.endsWith(userSchema.tableName) );
      const hasForeignKeyColumn = !!columns && columns.find(o=>o['column_name']==belongsToSchema.foreign_key)

      if (!hasForeignKeyColumn) {
        return false
      }
      
      // if List query table has belongsToSchema.foreign_key column, 
      // alias table and fields for List query, and add belongsTo join to userSchema.tableName
      if (isAlreadyJoined){
        // console.log(`a>> ${req.route.path}`, queryParamsObj);
        // already joined, so just get the alias for the belongsToTable
        let [alias, foundTableName] = isAlreadyJoined.split('.')
        if (alias===userSchema.tableName) {
          // console.log(`b>> ${req.route.path}`, queryParamsObj);
          // add alias to userSchema.tableName
          joinTables = joinTables.map( v=>v.endsWith(userSchema.tableName) ? `${userSchema.alias}.${userSchema.tableName}` : v )
          req.query._join = joinTables
        }
        else if (foundTableName===userSchema.tableName) {
          // console.log(`c>> ${req.route.path}`, queryParamsObj);
          // just update userSchema.alias
          belongsToSchema.alias = alias;
        }
        this._ownerAccess_fields(req, tableName, queryParamsObj, belongsToSchema)
      }
      else if (isUserSchemaTable){
        // console.log(`d>> ${req.route.path}`, joinTables, tableName, queryParamsObj);
        this._ownerAccess_fields(req, tableName, queryParamsObj, this.userSchema)
        this._ownerAccess_join(req, tableName, queryParamsObj, oid, this.userSchema)
      }
      else {
        // console.log(`e>> ${req.route.path}`, queryParamsObj);
        // console.log(">>> join owner table =", queryParamsObj.query)
        this._ownerAccess_fields(req, tableName, queryParamsObj)
        this._ownerAccess_join(req, tableName, queryParamsObj, oid)
      }

      if (!!req.query._where){

        // add alias prefix to belongsToSchema.table column names in the existing WHERE clause
        let whereClauseObj = whereHelp.getConditionClause(req.query._where);
        // console.log("### WHERE=",whereClauseObj)

        let isUserSchemaTable = tableName===this.userSchema.tableName
        let column_alias = isUserSchemaTable ? this.userSchema.alias : belongsToSchema.alias;

        const substitutions = whereClauseObj.params.reduce( (o, v)=>{
          if (columns && columns.find(o=>o['column_name']==v)) {  
            // add alias [tableName] as fk to column names
            // WARN: column names become RESERVED words
            o[v] = `${column_alias}.${v}`;
          }
          return o;
        }, {} )
        // console.log("\n### substitutions=", substitutions, req.query._where);
        for (let [k,v] of Object.entries(substitutions)) {
          // console.log(`   >  BEFORE: entry=${[k,v]}, str=` , req.query._where);
          let re = new RegExp(`\\b(?<!\\.)(${k})\\,`,"ig");
          req.query._where = req.query._where.replace(re,`${v},`);
          // let re = new RegExp(`\\((${k})\\,`,"ig");
          // req.query._where = req.query._where.replace(re,`(${v},`);
          // console.log("   >  AFTER", req.query._where)
        }
        // console.log("_ownerAccess_where()=", req.query._where, queryParamsObj);
      }
  ...
  _ownerAccess_fields(req, fkTable, queryParamsObj, belongsToSchema=null){
    const userSchema = this.userSchema
    if (!belongsToSchema) belongsToSchema = Object.assign({}, this.belongsToSchema)  // clone
    
    const fields = req.query._fields && req.query._fields.split(",");
    const fkColumns = this.metaDb.tables[fkTable]["columns"]
    if (!!fields) {
      // prefix fields with belongsToTable alias, 
      // skip last param, should be belongsToTable
      queryParamsObj.params = queryParamsObj.params.map((v,i,l)=>{
        let isNotLastValue = i+1<l.length;  // last value is tableName
        let isColumnFromTable = fkColumns.find(o=>o['column_name']==v)
        return isNotLastValue && isColumnFromTable ? `${belongsToSchema.alias}.${v}` : v
      });
    }
    else {
      if (queryParamsObj.query.includes(' * ')){
        // console.log(">>> * ", belongsToSchema)
        queryParamsObj.query = queryParamsObj.query.replace(' * ', ` ${belongsToSchema.alias}.* `)
      }
    }
  }

  _ownerAccess_join(req, fkTable, queryParamsObj, oid, belongsToSchema=null){
    const userSchema = this.userSchema
    if (!belongsToSchema) belongsToSchema = Object.assign({}, this.belongsToSchema)  // clone

    let isSelfJoin = fkTable===this.userSchema.tableName
    if (isSelfJoin) {
      queryParamsObj.query += " as ?? ";
      queryParamsObj.params = queryParamsObj.params.concat( [
        belongsToSchema.alias,
      ]);

      // add WHERE clause to restrict to owner_id
      // example: `where (m.member_id='[oid]')`
      this.getWhereClause(
        `(${belongsToSchema.alias}.${userSchema.key},eq,${oid})`,
      ...
      // add belongsTo join
      // example: `join members as m on (fk.member_id=m.member_id) and (m.member_id='[oid]')`
      queryParamsObj.query += " as ?? join ?? as ?? on (??=??) and (??=?) "
      queryParamsObj.params = queryParamsObj.params.concat( [
        belongsToSchema.alias,          
        userSchema.tableName, userSchema.alias, 
        `${belongsToSchema.alias}.${belongsToSchema.foreign_key}`, `${userSchema.alias}.${userSchema.key}`,
        `${userSchema.alias}.${userSchema.key}`, oid,
      ])
    }
  }
  ...
  prepareJoinQuery(req, res, queryParamsObj, oid = null) {
    queryParamsObj.query = "SELECT ";
    ...
    while (1) {
      ...
      if (queryParamsObj.grammarErr) {
        ...
        // find the join table with a belongsTo user relationship, and add owner access restriction
        const joinTables = req.query._join && req.query._join.split(",");
        for (let tableAlias of joinTables) {
          let tableName = tableAlias.split('.').pop();
          let done = this._ownerAccess_where(req, tableName, queryParamsObj, oid)
          if (done) {
            // console.log(`\nxxjoin >>> ${req.route.path}`, queryParamsObj);
            break;
          }
        }
    ...
    // console.log(">>> XJOIN query=", queryParamsObj)
    return queryParamsObj;
  }


  /**
   * special DB query to get all counts related to a user
   *    SQL template is saved in dotenv COUNT_SQL and takes a ownerId as param
   * @param {*} req 
   * @param {*} res 
   * @param {*} queryParamsObj 
   * @param {*} oid 
   */
  prepareCountQuery(req, res, queryParamsObj, oid = null) {
    queryParamsObj.query = process.env.COUNT_SQL;
    queryParamsObj.params = [ oid = oid || this.oid ];
    return queryParamsObj;
  }
