...
class Xsql {
  ...
  _ownerAccess_where(req, tableName, queryParamsObj, oid=null){
    ...
    if (!!oid) {
      ...
      if (isAlreadyJoined){
        ...
        this._ownerAccess_join(req, tableName, queryParamsObj, this.userSchema)
      }
      else {
        ...
        this._ownerAccess_join(req, tableName, queryParamsObj)
      }

      // add WHERE clause to restrict to owner_id
      this.getWhereClause(
        `(${userSchema.alias}.${userSchema.key},eq,${oid})`,
        tableName,
        queryParamsObj
      );
  ...
  _ownerAccess_join(req, fkTable, queryParamsObj, belongsToSchema=null){
    const userSchema = this.userSchema
    if (!belongsToSchema) belongsToSchema = Object.assign({}, this.belongsToSchema)  // clone
    ...
    if (isSelfJoin) {
      ...
      // add belongsTo join
      queryParamsObj.query += " as ?? join ?? as ?? on (??=??) "
      queryParamsObj.params = queryParamsObj.params.concat( [
        belongsToSchema.alias,          
