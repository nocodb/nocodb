            ...
            <v-list dense expand>
              ...
              <template v-for="item in listViewArr" v-if="showNode(item)">
                ...
                >
                  ...
                  <template v-slot:activator>
                    ...
                    <v-list-item-title>
                      <v-tooltip top v-if="!isNonAdminAccessAllowed(item)">
                        ...
                        <template v-slot:activator="{on}">
                          <span v-on="on" class="caption font-weight-regular"> {{ item.name }}</span>

                        </template>
                        <span class="caption">Only visible to Creator</span>
                      </v-tooltip>
                      <span v-else class="caption font-weight-regular"
                            @dblclick="showSqlClient = true"> {{ item.name }}</span>
                    ...
                    <v-tooltip bottom>
                      ...
                      <span class="caption">Add new <span class="text-capitalize">{{
                          item.type.slice(0, -3)
                        }}</span></span>
                  ...
                  <v-list-item-group :value="selectedItem">
                    <v-list-item
                      ...
                      :key="child.key"
                      :value="`${(child._nodes && child._nodes).type || ''}||${(child._nodes && child._nodes.dbAlias) || ''}||${child.name}`"
                      class="nested ml-3 "
                    ...
                    >
                      <v-list-item-icon>
                        ...
                        <v-icon
                          x-small
                          style="cursor:auto;"
                          v-else :color="icons[child._nodes.type].color"
                        >{{ icons[child._nodes.type].icon }}
                        </v-icon>
                      </v-list-item-icon>
                      <v-list-item-title>
                        <v-tooltip bottom v-if="_isUIAllowed('creator_tooltip') && child.creator_tooltip">
                          <template v-slot:activator="{on}">
                      ...
                      <template v-if="child.type==='table'">
                        ...
                        <div class="action d-flex" @click.stop>
                          <v-menu>
                            ...
                            <v-list dense>
                              <v-list-item dense v-if="_isUIAllowed('treeview-rename-button')"
                                           @click="
                          menuItem = child;
                          ...
                          dialogRenameTable.defaultValue = child.name;">
                              ...
                              <v-list-item dense v-if="_isUIAllowed('ui-acl')"
                                           @click="openUIACL">
                                <v-list-item-icon>
                                  <v-icon
                                    x-small>
                ...
                <v-list-item
                  ...
                  :key="item.key"
                  :value="`${(item._nodes && item._nodes).type || ''}||${(item._nodes && item._nodes.dbAlias) || ''}||${item.name}`"
            ...
            <v-list-item>
              <v-list-item-title>
                ...
                <v-tooltip top>
                  ...
                  <template v-slot:activator="{on}">
                    <x-icon class="mt-n1" v-on="on" color="pink grey"
            ...
            <v-list dense>
              ...
              <div class="mx-4 d-flex align-center mb-2">
                <template v-for="(role,i) in rolesList">
                  ...
                  <div @click="setPreviewUSer(role.title)" :key="role.title" class="pointer text-center">
...
export default {
  components: {SponsorMini, DlgViewCreate, DlgTableCreate, textDlgSubmitCancel, dlgLabelSubmitCancel},
  data: () => ({
    roleIcon: {
  ...
  computed: {
    ...
    isTreeView() {
      return this.projects && (
        ...
        || (
          this.projects[0] && this.projects[0].children && (this.projects[0].children.length > 1
            || (this.projects[0].children[0] && this.projects[0].children[0].children && this.projects[0].children[0].children.length > 1)
          )
        )
      );
    },
  ...
  methods: {
        ...
        if (item._nodes.type === "tableDir" && !open) {
          ...
          const tabIndex = this.tabs.findIndex(el => {
            return (
                ...
                || (el._nodes && item._nodes && el._nodes.type === item._nodes.type && el._nodes.dbAlias === item._nodes.dbAlias)
              )
              && item.name === el.name
          });
          if (tabIndex !== -1) {
    ...
    isActiveList(item) {
      return item.type === this.$route.query.type
        || item.type === `${this.$route.query.type}Dir`;
    },
    showNode(item) {
      return !(this.$store.getters['project/GtrProjectPrefix'] && ['functionDir', 'procedureDir'].includes(item.type))
    ...
    isNested(item) {
      return (item.children && item.children.length) || ['tableDir',
        'viewDir',
        'functionDir',
        'procedureDir',
        'sequenceDir'].includes(item.type)
    },
...
/deep/ .advance-menu .v-list-item--dense, /deep/ .advance-menu .v-list--dense .v-list-item {
  min-height: 32px;
}

/deep/ .advance-menu .v-list-item--dense .v-list-item__icon, /deep/ .advance-menu .v-list--dense .v-list-item .v-list-item__icon {
  margin-top: 4px;
  margin-bottom: 4px;
}
