<template>
  <div>
    ...
    <v-tabs v-model="tab" style="height: 100%" height="40">
      <v-tab v-for="(n, i) in terminals" :key="n" class="caption">
      ...
      <x-icon class="mx-2" small @click="addTerminal"> mdi-plus </x-icon>
      <span v-shortkey="['meta', 't']" @shortkey="addTerminal" />
      <span
        v-if="isModal"
        v-shortkey="['meta', 'w']"
        @shortkey="terminals.length && closeTerminal(tab)"
      />

      <div
        v-if="!isDocker"
        class="flex-grow-1 d-flex text-right pr-4 justify-end"
      >
        <x-icon
          v-if="_isWindows"
          icon-class="mr-4"
          ...
          <v-tooltip bottom>
            <template #activator="{ on }">
              <img
                ...
                src="~/assets/img/brand/favicon-64.png"
                v-on="on"
      ...
      <v-tab-item v-for="i in terminals" :key="i">
...
export default {
  ...
  methods: {
    ...
    addTerminal() {
      ...
      this.$nextTick(() => {
        this.initTerminal(
          this.$refs.term[this.terminals.length - 1],
    ...
    initTerminal($el, index) {
      try {
        ...
        this.$toast
          .error(
            'Error opening the terminal\n\nPlease use your system terminal'
          )
          .goAway(5000)
      }
    },
