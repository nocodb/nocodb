...
<template>
  <div class="nc-treeview-container flex flex-col">
    <a-menu v-model:openKeys="menu" mode="inline" class="flex-grow min-h-50 overflow-y-auto">
      <a-sub-menu
            ...
            >
              ...
              <div
                v-if="
                  ...
                  !bases.slice(1).filter((el) => el.enabled)?.length
                "
                class="flex-1"
              >
                <div
                    ...
                    <template #overlay>
                      <a-menu class="!py-0 rounded text-sm">
                        <a-menu-item-group class="!px-0 !mx-0">
                          ...
                          <a-menu-item key="table-magic" @click="openTableCreateMagicDialog(projects[project.id].bases[0].id)">
                            <div class="color-transition nc-project-menu-item group">
                              <MdiMagicStaff class="group-hover:text-accent" />
                              Create table
                            </div>
                          </a-menu-item>
                          <a-menu-item key="schema-magic" @click="openSchemaMagicDialog(projects[project.id].bases[0].id)">
                            <div class="color-transition nc-project-menu-item group">
                              <MdiMagicStaff class="group-hover:text-accent" />
                              Create schema
                            </div>
                    ...
                    <div v-if="projects[project.id].bases[0]" :key="`base-${projects[project.id].bases[0].id}`">
                      <div
                        ...
                        :key="`sortable-${projects[project.id].bases[0].id}-${
                          projects[project.id].bases[0].id && projects[project.id].bases[0].id in keys
                            ? keys[projects[project.id].bases[0].id]
                            : '0'
                        }`"
                        :nc-base="projects[project.id].bases[0].id"
                      >
                        <div
                          v-for="table of (projectTableList[project.id] ?? []).filter(
                            (table) => table.base_id === projects[project.id].bases[0].id,
                          )"
                          :key="table.id"
                          v-e="['a:table:open']"
                          :class="[
    ...
    <a-dropdown :trigger="['contextmenu']" overlay-class-name="nc-dropdown-tree-view-context-menu">
      <div class="pt-2 pl-2 pb-2 flex-1 overflow-y-auto flex flex-col scrollbar-thin-dull" :class="{ 'mb-[20px]': isSharedBase }">
        <div
          v-if=" projects[project.id].bases[0] && bases[0].enabled && !bases.slice(1).filter((el) => el.enabled)?.length"
          class="base-context min-h-[36px] py-1 px-3 flex w-full items-center gap-1 cursor-pointer"
