    ...
    ...(formState.value.dataSource.client === ClientType.SQLITE
      ...
      : {
          ...
          formState.value.dataSource.searchPath
            ? {
                'dataSource.searchPath.0': selectedIntegration.value ? [] : [fieldRequiredValidator()],
              }
            : {}),
        }),
  }
...
const changeIntegration = () => {
  if (formState.value.fk_integration_id) {
    formState.value.dataSource = {
      connection: { client: selectedIntegration.value.sub_type, database: selectedIntegrationDb.value },
  ...
  <div class="edit-source bg-white relative flex flex-col justify-start gap-2 w-full p-2">
    ...
    <a-form ref="form" :model="formState" name="external-base-create-form" layout="horizontal" no-style :label-col="{ span: 5 }">
      ...
      >
        ...
        <template v-if="formState.dataSource.client === ClientType.SQLITE || selectedIntegration?.sub_type === ClientType.SQLITE">
          <!-- SQLite File -->
          <a-form-item
