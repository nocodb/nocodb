...
const selectedCardsMap = ref<Record<string, boolean>>({})
</script>
    ...
    <div class="h-full flex flex-col">
      <div class="flex items-center justify-between gap-4 p-4 border-b-1 border-gray-200">
        ...
        <div class="flex-1 flex items-center gap-3">
          ...
          <NcBadge :border="false" class="text-primary !h-7 bg-brand-50 text-sm px-2 font-weight-500">{{ $t('title.comingSoon') }}</NcBadge>
        </div>
      ...
      <div class="flex-1 p-6 overflow-auto nc-scrollbar-thin">
        <div class="flex flex-col gap-6 max-w-[992px] mx-auto">
          ...
          <div class="flex items-start gap-3">
            <div
              class="nc-sync-data-card"
              :class="{
                selected: selectedCardsMap['1'],
              }"
            ></div>
          </div>
        </div>
...
.nc-project-sync-data-modal-wrapper {
  ...
  .nc-sync-data-card {
    @apply p-3 flex items-center gap-4 rounded-xl border-1 border-gray-200 w-[296px] h-[76px] cursor-pointer;
