<template>
  <v-container fluid class="api-client grid-list-xs pa-0 " style="height: 100%;">
      ...
      <pane min-size="20" max-size="50" size="30" style="overflow: auto">
        ...
        <v-row class="pa-0 ma-0 pa-2 pl-2">
          <div class=" cursor-pointer d-flex" style="width: 100%">
            <x-icon icon-class="mr-1 cursor-pointer"
                    small v-ge="['api-client','open-new-collection']" @click="openNewCollection('/')" color="primary"
            ...
            <v-dialog v-model="importCollection.modal" persistent max-width="550">
              ...
              <v-card class="pa-3" style="position: relative">
                ...
                <v-icon style="position: absolute;right:20px; top:20px;"
                        v-ge="['api-client','open-new-collection-close']" @click="importCollection.modal = false">
                  mdi-close
                </v-icon>
                    ...
                    <v-tabs background-color="indigo accent-4" dark style="border: 1px solid indigo ; margin:0 10px">
                      ...
                      <v-tab-item>
                        <div class="px-2 py-10 text-center" style="min-height: 250px;">
                          ...
                          <v-row>
                            <v-col>
                              <v-text-field hide-details label="Select file" dense outlined
                          ...
                          <v-row v-show="isSwaggerImport">
                            <v-col>
                              <v-text-field hide-details label="Destination file path"
                                            v-model="importCollection.file.dstFilePath" dense outlined
                      ...
                      >
                        <div style="min-height: 250px;" class="text-center">
                          ...
                          <div class="px-2 pt-10 ">
                            <v-text-field v-model="importCollection.url" placeholder="Enter url"></v-text-field>
                          </div>

                          <v-btn color="primary"
                                 v-ge="['api-client','import-collection-from-url']"
                                 @click="importFromUrl(importCollection.type,importCollection.url)">Import
                          </v-btn>
                        </div>
                      </v-tab-item>
                      <v-tab-item
                      >
                        <div style="min-height: 250px;" class="text-center">
                          <div class="px-2  pt-2 ">
                            <v-textarea v-model="importCollection.text"
                                        placeholder="Enter JSON String"></v-textarea>
                          </div>
                          <v-btn color="primary"
                                 v-ge="['api-client','import-collection-from-text']"
                                 @click="importFromText(importCollection.type,importCollection.text)">Import
                          </v-btn>
                        </div>
                      </v-tab-item>
            ...
            <v-spacer></v-spacer>
            <span class="flex-shrink-1 body-2 my-1 text-center grey--text" v-show="!isDashboard">
        ...
        <v-tabs height="32" v-model="apiRootTab" style="border-top:1px solid darkgrey">
          ...
          <v-tab-item style="border-top: 1px solid grey">
            <div class="apis-list">
              <div v-for="(api,i) in historyList" :key="i" class="pa-0 ma-0 ">
                <v-list-item dense two-line v-ge="['api-client','history']" @click="apiClickedOnHistoryList(api)">

                  <v-hover v-slot:default="{ hover }">
                    <v-list-item-content>
                      <v-list-item-title class="grey--text">
                        <v-btn class="pl-0 ml-0" small text :tooltip="api.meta.path"
                      ...
                      <v-list-item-subtitle v-show="!i || hover" class="text-right">
                        ...
                        <v-icon v-if="_isDev" small v-ge="['api-client','node-info']" @click.stop="showNodeInfo(api)">
                          mdi-information
                        </v-icon>
                        <v-btn small text v-show="hover" class=" " v-ge="['api-client','delete-list']"
                ...
                <v-expansion-panel-header hide-actions>
                  <template v-slot:default="{open}">
                    <div class="d-flex">
                      ...
                      <span
                        class="body-2 flex-grow-1">{{ $store.getters['apiClientSwagger/GtrCurrentApiFilePaths'][i].fileName }}</span>
                ...
                <v-expansion-panel-content class="expansion-wrap-0 pl-4">
                  <vue-tree-list
                    ...
                    <template v-slot:label="{item:api}">
                      <div v-if="api.isLeaf" class="d-flex pa-1 ma-n1" style="width: 100%" :style="
                        ...
                        <!--                            </v-icon>-->
                        <span style="display: inline-block;min-width: 45px;"
                        ...
                        <span class="grey--text d-block text--darken-1" style="text-overflow: ellipsis;
    overflow: hidden;
    white-space: nowrap;">{{ api.name }}</span>
                      ...
                      <div class="d-flex" v-else style="width: 100%;text-overflow: ellipsis;
    overflow: hidden;
    white-space: nowrap;">
                        <span>{{ api.name }}</span>

                        <v-spacer></v-spacer>
                        <v-icon v-if="_isDev" small @click.stop="showNodeInfo(api)" v-ge="['api-client','node-info']">
                          mdi-information
                        </v-icon>
                      </div>
                    </template>
      ...
      <pane min-size="10" size="70" style="overflow: auto">
        ...
        <v-toolbar class=" toolbar-border-bottom  elevation-0 d-flex req-inputs" height="55"
          ...
          <xAutoComplete
            ...
            v-model="api.path"></xAutoComplete>


          <x-btn btn.class="primary" dense @click.prevent="apiSend()" v-ge="['api-client','api-send']"
                 tooltip="Send Request">
            <v-icon small class="ml-n3" v-if="isPerfFilled">mdi-truck-fast</v-icon>
            <v-icon small class="ml-n3" v-else>mdi-send</v-icon>
            &nbsp;SEND
          </x-btn>
          <x-btn btn.class="outlined" dense v-ge="['api-client','save']" @click.prevent="saveOrUpdateApi(api)"
                 :icon="api.history ? 'save' : 'mdi-content-save-edit'"
                 tooltip="Save API">
          </x-btn>
          <x-btn icon="mdi-eye-outline"
        ...
        <splitpanes horizontal style="height:calc(100% - 64px)" class="xc-theme">
          <pane min-size="25" size="50" style="overflow: auto;" class="pa-1">
            ...
            <v-tabs
              ...
              height="24"
            >
              <v-tab v-ge="['api-client','params']" class="caption">Params&nbsp;<b v-if="paramsCount"
                                                                                   class="green--text">({{ paramsCount }})</b>
              </v-tab>
              <v-tab class="caption" v-ge="['api-client','headers']">Headers&nbsp;<b v-if="headersCount"
                                                                                     class="green--text">({{
                  headersCount
                }})</b>
              </v-tab>
              <v-tab class="caption" v-ge="['api-client','body']">Body</v-tab>
              <v-tab class="caption" v-ge="['api-client','auth']">Auth</v-tab>
              <v-tab class="caption" v-ge="['api-client','perf-test']">Perf Test</v-tab>
          ...
          <pane min-size="25" size="50" style="overflow: auto" class="pa-1">
            ...
            >
              ...
              <v-tab v-ge="['api-client','respomse-headers']" class="caption">Headers<span class="green--text"
                                                                                           v-if="api.response.headers">( {{ Object.keys(api.response.headers).length }} )</span>
              </v-tab>
              <div class="flex-grow-1 text-right pr-4 caption" v-if="api.response">
                ...
                &nbsp;
                &nbsp;
                <template v-if="api.timeTaken"><span class="grey--text">Time:</span><span
                  class="green--text"><b>{{ api.timeTaken }}ms</b></span>
                </template>


              </div>
              <v-tab-item>
                <pre v-if="api.response" class="black pa-1 grey--text w-100 caption"
                      style="overflow-x: auto;min-height:100px;overflow-y:auto;min-width: 100%">{{ api.response.data }}</pre>

                <!--                <pre v-if="api.response" class="black pa-1" style="overflow-x: auto;min-height:50px;overflow-y:auto">{{api.response.data}}</pre>-->
              </v-tab-item>
              <v-tab-item>
                <pre v-if="api.response" class="black pa-1 grey--text w-100 caption"
                      style="overflow-x: auto;min-height:100px;overflow-y:auto;min-width: 100%">{{ api.response.headers }}</pre>
              </v-tab-item>
            </v-tabs>
          </pane>
        </splitpanes>
    ...
    <environment env="dev" v-model="environmentDialog"></environment>
      ...
      <v-card>
        ...
        <v-card-text>
          <div class="text-right">
            <v-btn small color="primary" outlined v-ge="['api-client','open-new-collection']"
          ...
          <v-list dense>
            ...
            <v-list-item-group v-model="choosenApiCollection">
              <v-list-item v-for="(item,i)  in $store.getters['apiClientSwagger/GtrCurrentApiFilePaths']" :value="i"
                           :key="i"
        ...
        <v-card-actions class="justify-center pb-5">
          <v-btn small v-ge="['api-client','bookmark']" @click="bookmarkApiDialog = false">Cancel</v-btn>
          <v-btn small color="primary" :disabled="choosenApiCollection === null  || choosenApiCollection === undefined"
                 @click="bookmarkApi(choosenApiCollection)">Save API
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  methods: {
    ...
    async openNewCollection() {
      try {
        let toLocalPath = this.isDashboard ? path.join(this.currentProjectFolder, 'server', 'tool', this.projectApisFolder) : '';

        let userChosenPath = dialog.showSaveDialog({
    ...
    async openSwaggerJSONFile(srcPath, dstPath) {
      let dstFileName = path.basename(dstPath);
    ...
    selectImportFile() {
      ...
      let dstFileName = `${path.basename(file[0], '.json')}.xc.json`;
    ...
    async importSwaggerFromUrl(url) {
      ...
      let dstFileName = `${Date.now()}.xc.json`;
    ...
    async apiSend() {
      ...
      let swaggerResult = await this.$store.dispatch('apiClientSwagger/send', {api: this.api, apiDecoded});

      this.historyList = [{...this.currentApi, meta: swaggerResult, pid: 0}, ...this.historyList];

      let result = swaggerResult.response;
    ...
    async tvNodeRequestAdd(index) {
      var node = new TreeNode({name: 'New Request', isLeaf: true})
      if (!this.apiTvs[index].children) this.apiTvs[index].children = []
      this.apiTvs[index].addChildren(node);
    ...
    openApiFileCollection(filePath = null) {
      ...
      const file = filePath ? [filePath] : dialog.showOpenDialog({
        properties: ["openFile"]
      });
    ...
    async loadFileCollection(path, index = 0) {
      ...
      try {
        ...
        let treeData = await fileCollection.read();

        /* create a tree view and push it */
        let data = new Tree(treeData.length && treeData[0].name === 'swagger.json' ? treeData[0].children : treeData);
    ...
    async saveOrUpdateApi() {
      if (this.api.history) {
        ...
        let activePanelIndex = this.curApiCollectionPanel;
