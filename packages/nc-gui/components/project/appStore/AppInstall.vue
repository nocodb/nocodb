    ...
    <v-form v-model="valid">
      <template v-if="formDetails">
        <div class="title d-flex align-center justify-center mb-4">
          ...
          >
            <img :src="plugin.logo" height="25" />
          </div>
          <v-icon v-else-if="plugin.icon" color="#242f3e" size="30" class="mr-1">
            {{ plugin.icon }}
          </v-icon>
            ...
            <tbody>
              ...
              <tr>
                <td :colspan="formDetails.items.length" class="text-center">
                  <x-icon icon-class="pointer" @click="settings.push({})"> mdi-plus </x-icon>
                </td>
              </tr>
            </tbody>
          </table>
