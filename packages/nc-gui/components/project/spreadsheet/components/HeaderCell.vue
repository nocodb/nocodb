<template>
  <div class="d-flex align-center d-100">
    ...
    <span class="name" style="white-space: nowrap" :title="value">{{
      value
    }}</span>

    <span
      v-if="(column.rqd && !column.cdf) || required"
    ...
    <v-menu
      v-if="
        !isLocked && !isPublicView && _isUIAllowed('edit-column') && !isForm
      "
      open-on-hover
      left
      z-index="999"
    ...
    >
      <v-card class="nc-delete-dialog-card">
        ...
        <v-card-text class="mt-4 title">
          ...
          <span class="font-weight-bold">'{{ column.title }}'</span> column ?
        </v-card-text>
        <v-divider />
