...
import { type ColumnType, type InterfaceKanbanVizTheme, PermissionEntity, PermissionKey, isVirtualCol } from 'nocodb-sdk'
...
/**
 * Present when mounted inside an interface page. The interface tree provides
 * `ReadonlyInj` from the viz config's `edit_inline` flag — inline writes (incl.
 * dragging cards between stacks) are gated on it instead of role checks alone.
 */
...
// Record add/delete affordances — interfaces gate these via add_delete_inline,
// orthogonally to card editing (edit_inline / ReadonlyInj).
...
const handleCardDragEnd = (e: any) => {
  ...
  // Height-collapse the source residue, then drop it from the DOM
