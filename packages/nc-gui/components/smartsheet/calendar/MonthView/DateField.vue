...
const calendarData = computed(() => {
  ...
  const firstDayToDisplay = dayjs(startOfMonth.startOf('week')).add(firstDayOffset, 'day')
  const today = dayjs()
      ...
      if (!endCol && startCol) {
        ...
        while (
          ...
          if (maxVisibleDays.value === 5 && [0, 6].includes(startDate.day())) {
            currentWeekStart = dayjs(currentWeekStart.add(1, 'week'))
            continue
          }
              ...
              <template #overlay>
                <div class="bg-nc-background-default px-4 gap-3 flex flex-col py-4 max-h-70 overflow-y-auto">
                  <LazySmartsheetCalendarSideRecordCard
                    ...
                    :draggable="false"
                    class="w-64"
                    :from-date="dayjs(record.row[record.rowMeta.range.fk_from_col.title!]).format('D MMM • h:mm A')"
                    :invalid="false"
                    :row="record"
                    :to-date="record?.rowMeta?.range?.fk_to_col?.title && record.row[record.rowMeta.range!.fk_to_col.title!] ?  dayjs(record.row[record.rowMeta.range!.fk_to_col.title!]).format('DD MMM • HH:mm A') : null"
