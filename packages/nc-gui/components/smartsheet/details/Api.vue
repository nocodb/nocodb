  ...
  () =>
    new HTTPSnippet({
      ...
      headers: [
        {
          name: 'xc-token',
          value: `<CREATE YOUR API TOKEN FROM : ${location.origin + location.pathname}#/account/tokens>`,
          comment: 'API token',
        },
      ],
      url: apiUrl.value,
...
const api = new Api({
  ...
  headers: {
    "xc-token": "<CREATE YOUR API TOKEN FROM : ${location.origin + location.pathname}#/account/tokens>"
  }
})
