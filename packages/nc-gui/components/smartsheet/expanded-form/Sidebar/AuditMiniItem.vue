...
<template>
  <div v-for="columnKey of columnKeys" :key="columnKey" class="p-2">
    <div class="flex items-center gap-1 !text-gray-600 text-xs nc-audit-mini-item-header">
    ...
    <div class="flex items-center gap-2 mt-2 flex-wrap">
      <template v-if="meta[columnKey]?.field_type === 'Attachment'">
        <div v-if="oldData[columnKey]?.length > 0" class="text-sm text-red-500 border-1 border-red-500 rounded-md px-1 py-1 bg-red-50 w-full">
        ...
        <div v-if="newData[columnKey]?.length > 0" class="text-sm text-green-700 border-1 border-green-500 rounded-md px-1 py-1 bg-green-50 w-full">
          <div class="flex flex-col items-start gap-2">
      ...
      <template v-else-if="meta[columnKey]?.field_type === 'LongText'">
        <div v-if="!!oldData[columnKey]" class="text-sm text-red-500 border-1 border-red-500 rounded-md px-1 py-0.25 bg-red-50 line-through">
          {{ oldData[columnKey] }}
        </div>
        <div v-if="!!newData[columnKey]" class="text-sm text-green-700 border-1 border-green-500 rounded-md px-1 py-0.25 bg-green-50">
          {{ newData[columnKey] }}
        </div>
      </template>
      <template v-else>
        <div v-if="!!oldData[columnKey]" class="nc-audit-mini-item-cell text-sm text-red-500 border-1 border-red-500 rounded-md px-1 py-0.25 bg-red-50 line-through">
          <SmartsheetCell
            :column="{
        ...
        <div v-if="!!newData[columnKey]" class="nc-audit-mini-item-cell text-sm text-green-700 border-1 border-green-500 rounded-md px-1 py-0.25 bg-green-50">
          <SmartsheetCell
            :column="{
              uidt: meta[columnKey]?.field_type,
              meta: normalizeMeta(meta[columnKey]?.meta),
