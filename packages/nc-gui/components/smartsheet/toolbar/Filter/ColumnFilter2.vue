      ...
      <template v-for="(filter, i) in filters" :key="i">
        <template v-if="filter.status !== 'delete'">
          ...
          <div v-else class="flex flex-row gap-x-0 w-full nc-filter-wrapper" :class="`nc-filter-wrapper-${filter.fk_column_id}`">
            ...
            <SmartsheetToolbarFilterLogicalOpSelect
              v-else
              v-model="filter.logical_op"
              :filter="filter"
              :link="link"
              :web-hook="webHook"
              :read-only="(visibleFilters.indexOf(filter) > 1 && !isLogicalOpChangeAllowed) || readOnly"
            ...
            <template v-else>
              ...
              <SmartsheetToolbarFilterComparisonOpSelect
                ...
                :uidt="types.value[filter.fk_column_id!]"
                :web-hook="webHook"
                :read-only="readOnly"
                :is-locked-view="isLockedView"
                :show-null-and-empty-in-filter="baseMeta.showNullAndEmptyInFilter"
              ...
              <SmartsheetToolbarFilterComparisonSubOpSelect
                ...
                v-model="filter.comparison_sub_op"
                :filter="filter"
                :column="getColumn(filter)"
                :uidt="types.value[filter.fk_column_id!]"
                :link="link"
