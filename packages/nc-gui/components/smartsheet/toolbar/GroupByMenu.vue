...
import { RelationTypes, UITypes, ViewLockType, ViewSettingOverrideOptions, ViewTypes, isLinksOrLTAR, isSystemColumn } from 'nocodb-sdk'
      ...
      >
        ...
        >
          ...
          <div v-if="!isPersonalViewNonOwner" class="flex items-center justify-between mt-2 empty:hidden">
            <NcDropdown
              v-if="availableColumns.length && fieldsToGroupBy.length > totalGroupByCount && totalGroupByCount < groupByLimit && !(view?.type === ViewTypes.TIMELINE && totalGroupByCount >= 1)"
