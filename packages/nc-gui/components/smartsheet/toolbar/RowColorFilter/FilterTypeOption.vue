...
interface Emits {
  (event: 'update:rowColoringMode', model: RowColoringMode): void
  (event: 'change', model: RowColoringMode): void
}
...
const chooseOption = (option: RowColoringMode) => {
...
<template>
  <template v-if="!rowColoringMode">
    ...
    >
      <a-button :class="[buttonClass]" type="text" @click="chooseOption('SELECT')">
      ...
      <a-button :class="[buttonClass]" type="text" @click="chooseOption('FILTER')">
        <div class="flex flex-col gap-1">
          <div class="flex gap-2 items-center">
            <GeneralIcon class="w-[16px] h-[16px]" icon="singleSelect" />
  ...
  <template v-else-if="rowColoringMode === 'FILTER'">
    <slot name="filter"></slot>
  </template>
  <template v-else-if="rowColoringMode === 'SELECT'">
