...
<template>
  <NcMenu v-if="view" :data-testid="`view-sidebar-view-actions-${view!.alias || view!.title}`" class="!min-w-70"
    data-id="toolbar-actions">
    <NcTooltip>
      <template #title> {{ $t('labels.clickToCopyViewID') }} </template>
      <div class="flex items-center justify-between p-2 mx-1.5 rounded-md cursor-pointer hover:bg-gray-100 group"
        @click="onViewIdCopy">
    ...
    <template v-if="!view?.is_default && isUIAllowed('viewCreateOrEdit')">
      ...
      <template v-if="inSidebar">
        <NcMenuItem v-if="lockType !== LockType.Locked" @click="onRenameMenuClick">
          ...
          entity: view.type !== ViewTypes.FORM ? $t('objects.view').toLowerCase() :
          ...
          <template v-for="(dialog, type) in quickImportDialogs">
            <NcMenuItem v-if="isUIAllowed(`${type}TableImport`) && !isPublicView" :key="type"
              ...
              <div v-e="[
                ...
                ]" :class="{ disabled: lockType === LockType.Locked }" class="nc-base-menu-item">
        ...
        <template #title>
          <div v-e="[
            ...
            <div class="nc-base-menu-item flex !flex-shrink group !py-1 !px-1 rounded-md bg-brand-50">
              <LazySmartsheetToolbarLockType :type="lockType"
                class="flex nc-view-actions-lock-type !text-brand-500 !flex-shrink" hide-tick />
            </div>
    ...
    <template v-if="table?.base_id && currentSourceId">
      <LazyDlgQuickImport v-for="tp in quickImportDialogTypes" :key="tp" v-model="quickImportDialogs[tp].value"
        :import-data-only="true" :import-type="tp" :base-id="table.base_id" :source-id="currentSourceId" />
    </template>
  </NcMenu>
  <span v-else></span>
</template>
