...
const { quickImportType, baseTemplate, importData, importColumns, importDataOnly, maxRowsToParse, baseId, sourceId, serverAttachment } =
  defineProps<Props>()
...
// Server-side file import via job (CSV, Excel, JSON)
async function importViaJob() {
  ...
  try {
    ...
    for (const table of data.tables) {
      ...
      const jobResult = await $api.instance.post(`/api/v1/db/file-import/${base.value.id}`, jobPayload)
      ...
      await new Promise<void>((resolve, reject) => {
        $poller.subscribe(
          { id: jobId },
          (pollerData: { id: string; status?: string; data?: { error?: { message: string }; message?: string; result?: any } }) => {
