...
export function useInfiniteData(args: {
  ...
  function passesRowFilters(data: Record<string, any> | undefined) {
    return validateRowFilters(
      [
        ...allFilters.value,
        ...computedWhereFilter.value,
        // Interface pages: the builder filters play the classic view filters' role here
        ...flattenFiltersForEval(interfaceDataApi?.builderScopeFilters?.() ?? []),
      ],
      data,
  ...
  async function updateOrSaveRow(
    ...
    const isValidationFailed = !passesRowFilters(data)
  ...
  // Interface pages: a link change's realtime frame carries no link fields, so it can't tell
  // whether the row still matches the page filter — re-read the row and re-check it instead.
  async function refreshInterfaceRow(rowId: string) {
    if (!interfaceDataApi?.fetchRecord) return

    const dataCaches = isGroupBy.value ? Array.from(groupDataCache.value.values()) : [getDataCache()]
    const found = findCachedRowByPk(dataCaches, rowId)
    if (!found) return

    let record: Record<string, any> | null = null
    try {
      record = await interfaceDataApi.fetchRecord(rowId)
    } catch {
      return
    }
    if (!record) return
    ...
    cachedRow.rowMeta.isValidationFailed = !passesRowFilters({ ...cachedRow.row, ...record })

    // Link cells keep their grid-shaped value (the link store already updated it) — the read
    // returns them in another shape. Everything else, e.g. formulas over the link, is refreshed.
    const refreshed = Object.fromEntries(
      Object.entries(record).filter(([title]) => {
        const col = meta.value?.columns?.find((c) => c.title === title)
        return col && !isLinksOrLTAR(col)
      }),
    )
    Object.assign(cachedRow.row, refreshed)
    Object.assign(cachedRow.oldRow, refreshed)
    callbacks?.syncVisibleData?.()
  }

  const interfaceRowRefreshHandler = (event: SmartsheetStoreEvents, payload?: { rowId?: string }) => {
    if (event !== SmartsheetStoreEvents.INTERFACE_ROW_REFRESH || !payload?.rowId) return
    refreshInterfaceRow(payload.rowId)
  }

  if (interfaceDataApi) {
    eventBus.on(interfaceRowRefreshHandler)
    onBeforeUnmount(() => eventBus.off(interfaceRowRefreshHandler))
  }
