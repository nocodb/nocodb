...
<template>
  <div class="flex flex-col">
    <NcPageHeader>
      <template #icon>
        <GeneralIcon icon="ncLock" class="flex-none text-[20px] h-5 w-5" />
      </template>
      <template #title>
        <span data-rec="true">
    ...
    <div class="nc-content-max-w p-6 h-[calc(100vh_-_100px)] flex flex-col gap-6 overflow-auto nc-scrollbar-thin">
      <div class="flex flex-col items-center" data-test-id="nc-authentication">
        <div class="flex flex-col w-150">
              ...
              <template #overlay>
                <NcMenu variant="small">
                  ...
                  <template v-if="googleProvider.id">
                    <NcDivider />
                    <NcMenuItem danger data-test-id="nc-google-delete" @click="deleteProvider(googleProvider.id)">
          ...
          <div class="mt-5 flex flex-col border-1 rounded-2xl border-nc-border-gray-medium p-6">
                ...
                <NcDropdown :trigger="['click']" overlay-class-name="!rounded-md" @click.stop>
                  ...
                  <template #overlay>
                    <NcMenu variant="small">
                      ...
                      <NcMenuItem disabled data-test-id="nc-saml-duplicate" @click="duplicateProvider(sam.id)">
                        <GeneralIcon icon="copy" />
                        {{ $t('general.duplicate') }}
                      </NcMenuItem>
                      <NcDivider />
                      <NcMenuItem danger data-test-id="nc-saml-delete" @click="deleteProvider(sam.id)">
                        <GeneralIcon icon="delete" />
                        {{ $t('general.delete') }}
                      </NcMenuItem>
                    </NcMenu>
          ...
          <div class="mt-5 flex flex-col border-1 rounded-2xl border-nc-border-gray-medium p-6" data-test-id="nc-oidc-provider">
            <div class="flex font-bold justify-between text-base" data-rec="true">
              {{ $t('labels.oidc') }}
            ...
            <a-divider class="text-gray-200" />
            <div class="flex flex-col">
              <span v-if="!oidcProviders.length" class="text-nc-content-gray-muted"> {{ $t('msg.info.noSaml') }} </span>
              <div
                ...
                <NcDropdown :trigger="['click']" overlay-class-name="!rounded-md" @click.stop>
                  ...
                  <template #overlay>
                    <NcMenu variant="small">
                      ...
                      <NcMenuItem disabled data-test-id="nc-oidc-duplicate" @click="duplicateProvider(oid.id)">
                        <GeneralIcon icon="copy" />
                        {{ $t('general.duplicate') }}
                      </NcMenuItem>
                      <NcDivider />
