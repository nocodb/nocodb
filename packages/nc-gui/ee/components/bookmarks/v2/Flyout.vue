...
<template>
  <div
    ...
    @keydown.stop
  >
    <BookmarksV2Header v-model:search="search" :is-empty="isEmpty" />

    <BookmarksV2CreateFolderRow v-if="isCreatingFolder" />

    <div class="nc-v2-body nc-scrollbar-thin">
      <div v-if="isLoading" class="nc-v2-loader">
        <GeneralLoader />
      </div>

      <div v-else-if="isEmpty && !isCreatingFolder" class="nc-v2-empty">
      ...
      <div v-else-if="!isEmpty && search.trim() && !filteredGroups.length" class="nc-v2-noresults">
        {{ $t('labels.noResults') }}
      </div>
