...
const { script, ...props } = defineProps<Props>()
...
watchEffect(
  () => {
    if (script?.description) formState.description = `${script.description}`
  ...
  <NcModal v-model:visible="dialogShow" size="small" :show-separator="false">
    <template #header>
      <div class="flex flex-row items-center gap-x-2">
        <GeneralIcon icon="ncScript" class="!w-5 !h-5" />
    ...
    <div class="mt-1">
      ...
      <div class="flex flex-row justify-end gap-x-2 mt-5">
        ...
        <NcButton
          ...
          size="small"
          :disabled="
            validateInfos?.description?.validateStatus === 'error' || formState.description?.trim() === script?.description
          "
          :loading="loading"
