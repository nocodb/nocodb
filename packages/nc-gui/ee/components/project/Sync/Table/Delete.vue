...
const { deleteSync } = tableSyncStore
...
const performDelete = async () => {
  ...
  $e('c:table-sync:delete:confirm', { dropTables: dropTables.value })
  try {
    await deleteSync(props.baseId, props.sync.id, { dropTables: dropTables.value })
    emits('deleted')
    vOpen.value = false
  } catch (e: any) {
...
<template>
  <NcModal v-model:visible="vOpen" :show-separator="false" size="small" centered>
    ...
    <div data-testid="nc-table-sync-delete-modal" class="flex flex-col">
      <div class="mb-3 text-nc-content-gray">
        {{ $t('msg.deleteSyncIntro', { name: sync.title }) }}
      </div>

      <a-radio-group v-model:value="dropTables" class="nc-table-sync-delete-options">
        <a-radio
          data-testid="nc-table-sync-delete-keep"
        ...
        <a-radio data-testid="nc-table-sync-delete-drop" :style="{ display: 'flex', alignItems: 'flex-start' }" :value="true">
          <div class="flex flex-col gap-0.5 leading-5">
            <span class="font-semibold text-nc-content-gray">
    ...
    <div class="flex flex-row mt-5 justify-end gap-x-2">
      ...
      <NcButton
        ...
        size="small"
        :loading="isDeleting"
        data-testid="nc-table-sync-delete-confirm"
        @click="performDelete"
      >
