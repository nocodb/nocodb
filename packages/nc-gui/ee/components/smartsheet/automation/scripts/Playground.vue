...
const findInputItemInPlayground = (
  items: any[],
  id: string
): { item: any; index: number; parent?: any } | null => {
...
<template>
  <div ref="playgroundContainer" :class="[
    {
    ...
    <div v-if="isRunningState || isFinishedState || playgroundData.length > 0"
      :class="['flex mx-auto flex-col gap-6', { 'pb-40': !compact, 'pb-4': compact }]">
      <div v-for="(item, index) in playgroundData" :key="index" class="playground-item" :data-type="item.type">
            ...
            <div v-if="item.content.children.length > 0" class="step-content p-4 space-y-4">
              <div v-for="(child, childIndex) in item.content.children" :key="childIndex">
                ...
                <template v-if="child.type === 'text'">
                  <div class="leading-5 whitespace-pre-wrap text-sm" :class="{
                ...
                <template v-else-if="child.type === 'table'">
                  <table class="nc-scripts-table">
                    <thead>
                      <tr>
                        <th v-for="(value, key) in child.content[0] || child.content" :key="key"
                    ...
                    <tbody>
                      <tr v-for="(row, rowIndex) in Array.isArray(child.content) ? child.content : [child.content]"
                        :key="rowIndex">
                        <td v-for="(value, key) in row" :key="key"
                          class="text-nc-content-gray-subtle leading-5 px-2 py-1">
                          {{ value }}
                        </td>
                      </tr>
                    </tbody>
                ...
                <template v-else-if="child.type === 'inspect'">
                  <VueJsonPretty :show-double-quotes="false" :highlight-selected-node="false" :show-line="false"
                    :deep="2" :data="child.content" />
                </template>
        ...
        <template v-else-if="item.type === 'text'">
          <div class="leading-5 whitespace-pre-wrap"
        ...
        <template v-else-if="item.type === 'table'">
          <table class="nc-scripts-table">
            <thead>
              <tr>
                <th v-for="(value, key) in item.content[0] || item.content" :key="key"
                  class="text-left font-semibold leading-5 p-2">
                  {{ key }}
                </th>
              </tr>
            ...
            <tbody>
              <tr v-for="(row, rowIndex) in Array.isArray(item.content) ? item.content : [item.content]"
                :key="rowIndex">
        ...
        <template v-else-if="item.type === 'inspect'">
          <VueJsonPretty :show-double-quotes="false" :highlight-selected-node="false" :show-line="false" :deep="2"
            :data="item.content" />
        </template>
    ...
    <div v-else-if="showRunButton && scriptStore" class="flex items-center flex-col gap-3 h-full justify-center">
      <NcTooltip :disabled="scriptStore.isValidConfig">
        <NcButton size="small" type="primary" :disabled="!scriptStore.isValidConfig" :loading="isRunningState"
    ...
    <div v-else-if="playgroundData.length === 0 && !showRunButton"
