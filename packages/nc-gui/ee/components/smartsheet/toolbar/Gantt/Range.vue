...
const { ganttRange, loadGanttData, ganttMetaData, viewMetaProperties } = useGanttViewStoreOrThrow()
...
const selectedFromCol = ref<string | undefined>(ganttRange.value?.[0]?.fk_from_col?.id)
const selectedToCol = ref<string | undefined | null>(ganttRange.value?.[0]?.fk_to_col?.id ?? null)
...
const saveRange = async () => {
  ...
  try {
    ...
    await $api.internal.postOperation(
      ...
      { operation: 'ganttViewUpdate', viewId: activeView.value.id },
      { gantt_range: range },
    )

    if (ganttMetaData.value) {
      ;(ganttMetaData.value as any).gantt_range = range
    }
...
const initialViewMode = computed({
  ...
  set: async (val: string) => {
    ...
    try {
      ...
      if (ganttMetaData.value) {
        ;(ganttMetaData.value as any).meta = newMeta
      }
    } catch (e: any) {
      message.error(await extractSdkResponseErrorMsg(e))
    }
...
<template>
  ...
  >
    <NcTooltip :disabled="!isToolbarIconMode" class="nc-gantt-btn">
      <template #title>
        {{ $t('activity.settings') }}
      </template>

      <NcButton
        v-e="['c:gantt:change-gantt-range']"
        class="nc-toolbar-btn !border-0 group !h-7"
        size="small"
        type="secondary"
        data-testid="nc-gantt-range-btn"
    ...
    <template #overlay>
      ...
      >
        <div v-if="calendarRangeDropdown" class="w-72 space-y-4 p-4" data-testid="nc-gantt-range-menu" @click.stop>
          <div class="flex flex-col w-full gap-2" data-testid="nc-gantt-range-option">
            ...
            <a-select
              ...
              data-testid="nc-gantt-range-from-field-select"
              :disabled="isLocked"
              @change="onFromChange"
              @click.stop
            >
            ...
            <div class="w-full space-y-2">
              <NcButton
                ...
                size="small"
                data-testid="nc-gantt-range-add-end-date"
                type="text"
                :shadow="false"
                :disabled="isLocked"
              ...
              <template v-else>
                ...
                <div class="flex">
                  <a-select
                    ...
                    :placeholder="$t('placeholder.notSelected')"
                    data-testid="nc-gantt-range-to-field-select"
                    dropdown-class-name="!rounded-lg"
                    @change="saveRange"
                    @click.stop
                  >
          ...
          <div class="flex flex-col w-full gap-2">
            ...
            <a-select
              ...
              dropdown-class-name="!rounded-lg"
              :disabled="isLocked"
              data-testid="nc-gantt-initial-view-select"
