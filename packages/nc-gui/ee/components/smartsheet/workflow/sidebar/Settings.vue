...
async function loadSubscribers() {
  if (!activeWorkspaceId.value || !activeProjectId.value || !workflow.value?.id) return

  isLoading.value = true
  try {
    const response = await api.internal.getOperation(activeWorkspaceId.value, activeProjectId.value, {
      operation: 'workflowListSubscribers',
...
async function addSubscribers() {
  ...
  try {
    await api.internal.postOperation(
      ...
      { operation: 'workflowAddSubscribers' },
      {
        workflowId: workflow.value.id,
...
async function removeSubscriber(subscriberId: string) {
  ...
  try {
    await api.internal.postOperation(
      activeWorkspaceId.value,
      activeProjectId.value,
      { operation: 'workflowRemoveSubscriber' },
      {
        workflowId: workflow.value.id,
        subscriberId,
      },
...
<template>
  <div class="flex flex-col h-full bg-nc-bg-default">
    ...
    <GeneralModal
      ...
      :mask-closable="false"
      :keyboard="!isSaving"
      wrap-class-name="nc-modal-add-workflow-subscribers"
      :footer="null"
      class="!w-[448px]"
      :closable="false"
    ...
    >
      <div>
        ...
        <div class="text-body text-nc-content-gray-subtle mb-5">
          Select users who will receive email notifications when this automation fails.
        </div>
