...
<template>
  <div class="flex flex-col" data-test-id="nc-authentication">
    <div class="flex-1 max-h-[calc(100vh_-_100px)] overflow-y-auto nc-scrollbar-thin flex flex-col items-center gap-6 p-6">
      <div class="flex flex-col gap-6 w-150">
              ...
              <NcDropdown :trigger="['click']" placement="bottomRight" @click.stop>
                ...
                <template #overlay>
                  <NcMenu variant="small">
                    ...
                    <NcMenuItem
                      class="!hover:bg-nc-bg-default !cursor-not-allowed"
        ...
        <div v-if="isScimFeatureEnabled" class="flex flex-col border-1 rounded-2xl border-nc-border-gray-medium p-6 gap-y-4">
          <div class="flex font-bold justify-between text-base items-center" data-rec="true">
            ...
            <NcButton v-else size="small" type="secondary" @click="handleUpgradePlan(PlanFeatureTypes.FEATURE_SCIM)">
      ...
      <DlgSAMLProvider
        v-if="samlDialogShow"
        v-model:model-value="samlDialogShow"
        is-workspace
        :is-edit="isEdit"
        :saml="providerProp"
      />
