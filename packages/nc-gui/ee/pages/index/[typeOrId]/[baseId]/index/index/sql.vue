...
const activeSqlEditor = computed(() => {
  if (base.value.id! in sqlEditors.value) {
    return sqlEditors.value[base.value.id!]
  } else {
    sqlEditors.value[base.value.id!] = {
      selectedBase: sources.value[0]?.id,
      sqlPrompt: '',
      rawSql: '',
    }
    return sqlEditors.value[base.value.id!]
...
const activePrompt = computed(() =>
  promptHistory.find((p) => activeSqlEditor.value.selectedBase === p.sourceId && p.prompt === activeSqlEditor.value.sqlPrompt),
)
...
const addHistory = (prompt: string, query: string, status: boolean | null = null, error = '') => {
  ...
  const fnd = promptHistory.find((p) => p.sourceId === activeSqlEditor.value.selectedBase && p.prompt === prompt)
  if (fnd) {
    fnd.query = query
    fnd.status = status
    fnd.error = error
    ...
    promptHistory.unshift({ sourceId: activeSqlEditor.value.selectedBase, prompt, query, status, error })
  }
}
...
const generateSQL = async () => {
  ...
  const source = sources.value.find((b) => b.id === activeSqlEditor.value.selectedBase)
  const baseTables = tables.value.filter((t) => t.source_id === source?.id && t.type === 'table' && t.table_name)
...
const runSQL = async () => {
  ...
  await $api.utils
    .selectQuery({
      sourceId: activeSqlEditor.value.selectedBase,
...
onMounted(() => {
  if (!activeTab.value) {
    addSqlEditorTab(base.value)
  }
  activeSqlEditor.value.selectedBase = activeSqlEditor.value.selectedBase || sources.value[0]?.id
})
</script>
    ...
    <div class="flex flex-col h-full w-full text-gray-60" :class="[historyDrawer ? 'col-span-9' : 'col-span-12']">
      <div class="flex flex-col h-[500px]">
        <div class="flex p-4">
          ...
          <MdiHistory
            v-if="loadMagic || promptHistory.filter((p) => p.sourceId === activeSqlEditor.selectedBase).length"
            id="history"
    ...
    >
      <div class="flex p-4">
        <a-steps progress-dot :current="-1" direction="vertical" size="small">
          <a-step
            v-for="pr of promptHistory.filter((p) => p.sourceId === activeSqlEditor.selectedBase)"
            :key="pr.prompt"
