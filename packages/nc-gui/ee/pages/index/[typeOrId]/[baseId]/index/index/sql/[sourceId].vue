...
const activeSqlEditor = computed(() => {
  ...
  if (source.value?.id && source.value.id in sqlEditors.value) {
    return sqlEditors.value[source.value.id]
  } else {
    sqlEditors.value[source.value.id] = { sqlPrompt: '', rawSql: '' }
    return sqlEditors.value[source.value.id]
  }
})

const activePrompt = computed(() =>
  promptHistory.find((p) => source.value?.id === p.sourceId && p.prompt === activeSqlEditor.value.sqlPrompt),
)
...
const addHistory = (prompt: string, query: string, status: boolean | null = null, error = '') => {
  if (!prompt.length || !source.value.id) return
  const fnd = promptHistory.find((p) => p.sourceId === source.value.id && p.prompt === prompt)
  if (fnd) {
    fnd.query = query
    fnd.status = status
    fnd.error = error
    ...
    promptHistory.unshift({ sourceId: source.value.id, prompt, query, status, error })
  }
}
...
const generateSQL = async () => {
  ...
  const baseTables = tables.value.filter((t) => t.source_id === source.value.id && t.type === 'table' && t.table_name)
...
function openSqlViewCreateDialog(sourceId?: string) {
  if (!sourceId && !sources.value.length) return
...
const changeBase = (sourceId?: string) => {
  if (sourceId) {
    const source = sources.value.find((b) => b.id === sourceId)
    if (source) {
      router.replace({ params: { sourceId: source.id } })
    } else {
      router.replace({ params: { sourceId: sources.value[0].id } })
    }
  }
}
    ...
    <div class="flex flex-col h-full w-full text-gray-60" :class="[historyDrawer ? 'col-span-9' : 'col-span-12']">
      <div class="flex flex-col h-[500px]">
        <div class="flex p-4">
          ...
          <MdiHistory
            v-if="loadMagic || promptHistory.filter((p) => p.sourceId === source?.id).length"
            id="history"
      ...
      <div class="flex-1 bg-slate-50 p-4">
        ...
        <a-button
          ...
          @click="openSqlViewCreateDialog(source?.id)"
        >
    ...
    >
      <div class="flex p-4">
        <a-steps progress-dot :current="-1" direction="vertical" size="small">
          <a-step
            v-for="pr of promptHistory.filter((p) => p.sourceId === source?.id)"
            :key="pr.prompt"
