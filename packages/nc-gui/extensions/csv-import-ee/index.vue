...
const onVerifyImport = async () => {
  ...
  const tableMeta = (await getMeta(selectedTable.value?.base_id as string, importPayload.value.tableId as string))!
...
async function onImport() {
  ...
  try {
    if (importPayload.value?.upsert) {
      ...
      const tableMeta = await getMeta(selectedTable.value?.base_id as string, importPayload.value.tableId)
      ...
      if (autoInsertOption.value) {
        await getMeta(selectedTable.value?.base_id as string, importPayload.value.tableId, true)
      }
    } else {
