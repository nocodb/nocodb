 ...
 */
export function ncArrayFrom<T>(length: number, contentCallback: (i: number) => T = i => `Item ${i + 1}` as unknown as T): T[] {
 ...
 */
export function getSafeInitials(title?: string, initialsLength: number = 2, limitEmojiToOne: boolean = false): string {
 ...
 * ```
 */
export function searchCompare(source?: NestedArray<string | number | undefined>, query?: string, onMatch?: (source: string | number | undefined) => void): boolean {
  if (ncIsArray(source)) {
 ...
 */
export function antSelectFilterOption(inputValue: string, option?: DefaultOptionType | NcListItemType, searchKey: keyof DefaultOptionType | keyof NcListItemType | (keyof NcListItemType)[] | (keyof DefaultOptionType)[] = 'key') {
 ...
 * console.log(name); // Output: 'John Doe'
 */
export function extractUserDisplayNameOrEmail(user?: UserType | Record<string, string>) {
  if (!user) return ''
...
export function pollUntil<T>(conditionFn: () => T | null | undefined | false, interval: number = 100): Promise<T> {
...
export function getDraggableAutoScrollOptions(params: Partial<AutoScrollOptions & { direction: SortableOptions['direction'] }> = {}): Partial<AutoScrollOptions & { direction: SortableOptions['direction'] }> {
  return {
    /**
 ...
 */
export function ncArrSortCallback<T>(a: T, b: T, sortOption: {
