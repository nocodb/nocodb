...
/**
 * When a Formula cell's rendered result is a single top-level URL (the whole cell is one
 * `URI::(url) LABEL::(label)` link, with no surrounding text and no `display_type` override),
 * produce richer clipboard representations:
 *   - `plain` → markdown `[label](url)` (bare `url` when there's no distinct label)
 *   - `html`  → clean anchor `<a href="url">label</a>`
 *
 * Returns `null` for anything that is not a strict top-level-URL formula cell (non-formula columns,
...
export const formatFormulaUrlForClipboard = (
  ...
  // Strict "whole cell is one URL": exactly one non-empty segment and it must be a link.
  const segments = getFormulaTextSegments(linkHtml).filter((seg) => seg.text?.trim())
  const segment = segments[0]
  if (segments.length !== 1 || !segment?.url) return null
