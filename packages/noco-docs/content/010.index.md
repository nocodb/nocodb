...
We provide different integrations in three main categories. See [App Store](/setup-and-usages/account-settings#app-store" target="_blank) for details.
...
We allow you to sync schema changes if you have made changes outside NocoDB GUI. However, it has to be noted then you will have to bring your own schema migrations for moving from environment to others. See [Sync Schema](/setup-and-usages/sync-schema" target="_blank) for details.

### Audit 

We are keeping all the user operation logs under one place. See [Audit](/setup-and-usages/audit" target="_blank) for details.
