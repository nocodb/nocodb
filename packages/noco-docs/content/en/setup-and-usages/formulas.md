...
|  |  | `IF(NOW() < DATEADD(date,10,'day'), "true", "false")` | If the current date is less than {DATE_COL} plus 10 days, it returns true. Otherwise, it returns false. | DateTime columns and negative values are supported. |
| **DATETIME_DIFF** | `DATETIME_DIFF(date, date, ["millisecond" \| "ms" \| "second" \| "s" \| "minute" \| "mi" \| "hour" \| "hh" \| "day" \| "dy" \| "week" \| "wk" \| "month" \| "m" \| "quarter" \| "q" \| "year" \| "y"])` | `DATETIME_DIFF("2017/08/25", "2011/08/25", "seconds")` | Supposing {DATE_COL_1} is 2017-08-25 and {DATE_COL_2} is 2011-08-25. The result is -189388800. | Compares two dates and returns the difference in the unit specified. Positive integers indicate the second date being in the future compared to the first and vice versa for negative ones. |
