...
| `NC_MINIMAL_DBS` | No | Enable minimal database feature of NocoDB, for more [details visit](/getting-started/self-hosted/environment-variables#Minimal-database -behavior). | If meta database used is Postgres then it will enable by default when database user have schema create premisiion |
...
This option helps to organize base data by breaking it into smaller, modular structures. Currently, this option works with PostgreSQL and SQLite databases.

- **SQLite**: Each base data is stored as a separate SQLite file, ensuring isolated data storage.
- **PostgreSQL**: A unique schema is created for each base, providing logical separation within the database.

**Note**: When using PostgreSQL, this option enables by default if the user has the required permissions. To disable this option set `NC_MINIMAL_DBS` environment variable to `false`.
