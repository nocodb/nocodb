...
Sync Dropbox employee, location, and employment data into the NocoDB HRIS schema. The package extends the `SyncIntegration` base class and streams normalized records through `DataObjectStream`.

## Features

- **HRIS schema coverage**: Populates `HRIS_EMPLOYEE`, `HRIS_LOCATION`, and `HRIS_EMPLOYMENT` tables.
- **Incremental sync**: Uses Dropbox `lastChanged` timestamps so subsequent runs only fetch updates.
...
- **Namespace isolation**: Stores the Dropbox company domain in `RemoteNamespace` for multi-domain datasets.

## How It Works

1. Requires a Dropbox Auth integration (`@noco-integrations/bamboohr-auth`) that yields an Axios client with the company domain and API key.
...
- Dropbox Auth integration configured with company domain and API key.
...
Fields populated by `DropboxFormatter.formatEmployee()`:
...
- Dropbox API rate limits still apply; stagger schedules for large tenants.
...
- Extend `DropboxFormatter` if additional HRIS fields are needed (for example, cost centers or custom fields).
