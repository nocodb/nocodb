...
interface UntagContactConfig extends WorkflowNodeConfig {
...
export class UntagContactNode extends WorkflowNodeIntegration<UntagContactConfig> {
  public async definition(): Promise<WorkflowNodeDefinition> {
    const form: FormDefinition = [
      ...
      },
      {
        ...
        helpText: 'Email address of the contact to untag',
        validators: [
          {
            type: FormBuilderValidatorType.Required,
      ...
      },
      {
        ...
        helpText: 'Comma-separated list of tags to remove from the contact',
        validators: [
          {
            type: FormBuilderValidatorType.Required,
    ...
    return {
      id: 'mailchimp.untag_contact',
      title: 'Untag contact',
      description: 'Remove tags from a contact in a Mailchimp audience',
      icon: 'ncMailchimp',
  ...
  public async run(
    ctx: WorkflowNodeRunContext<UntagContactConfig>,
    ...
    try {
      ...
      logs.push({
        level: 'info',
        message: `Tags removed from contact ${email} successfully`,
        ts: Date.now(),
      });
      ...
      logs.push({
        level: 'error',
        message: error.message || 'Failed to untag contact',
        ts: Date.now(),
      });

      return {
        outputs: { untagged: false },
        status: 'error',
        error: {
          message: error.message || 'Failed to untag contact',
