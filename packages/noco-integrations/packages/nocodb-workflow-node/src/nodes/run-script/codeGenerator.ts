...
export function generateRemoteFetch(): string {
  return `
...
export function generateV3ToV2Converter() {
  return `
    /**
    ...
    const convertLTARField = (fieldValue, field) => {
      ...
      // Handle array of linked records (hm, mm, om relations)
      ...
      // Handle single linked record (bt, oo, mo relations)
...
export function generalHelpers() {
  return `
...
export function generateBaseModels() {
  return `
  const UITypes = {
  ID: 'ID',
...
export function generateConsoleOutput(): string {
  return `
    ...
    console.log = (...args) => postMessage('${ScriptActionType.LOG}', { args });
    console.error = (...args) => postMessage('${ScriptActionType.ERROR}', { args });
    console.warn = (...args) => postMessage('${ScriptActionType.WARN}', { args });
    console.info = (...args) => postMessage('${ScriptActionType.LOG}', { args });
  `;
}

export function generateMessageHandler(userCode: string): string {
  return `
...
export function generateBaseObject(baseSchema: any): string {
  return `
  var base = new Base({
    id: '${baseSchema!.id}',
...
/**
 * Generate the output API that allows setting output values
 * Use output.set(key, value) to set outputs that downstream nodes can access
 */
export function generateOutputAPI(): string {
  return `
  const __nc_script_outputs = {};

  const output = {
    set: (key, value) => {
      if (!key || typeof key !== 'string') {
        throw new Error('output.set() requires a string key as the first parameter');
      }
      __nc_script_outputs[key] = value;
      postMessage('${ScriptActionType.OUTPUT_SET}', { key, value });
    }
  };
  `;
}
...
/**
 * Generate script message handler with output capture
 */
export function generateScriptMessageHandler(userCode: string): string {
  return `
    let ____script_error = false;
    let ____script_error_message = '';
    try {
        await (async () => {
          ${userCode}
        })();
        ...
        postMessage('${ScriptActionType.DONE}', {
          error: ____script_error,
          error_message: ____script_error_message,
          outputs: __nc_script_outputs
        });
      }
  `;
}
...
/**
 * Generate the complete script code for execution
 * Includes all NocoDB APIs: Base, Table, View, Field, Record, Collaborators, fetch, etc.
 */
export function createScriptExecutionCode(
  userCode: string,
  ...
  return `
    ...
    await (async () => {
      ...
      ${generalHelpers()}
      ${generateBaseModels()}
      ${context?.baseSchema ? generateBaseObject(context.baseSchema) : '// Base object not configured'}
      ${generateInputAPI(variableContext)}
      ${generateOutputAPI()}
      ${generateScriptMessageHandler(userCode)}
    })()
  `;
}
