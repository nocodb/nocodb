        ...
        for (const tableName of targetTables) {
          ...
          while (hasMore) {
            ...
            if (primaryKeys && primaryKeys.length > 0) {
              ...
              const orderByClause = primaryKeys.map(pk => `\`${pk}\` ASC`).join(', ');
            ...
            if (offset % 1000 === 0) {
              this.log(`[ClickHouse Sync] Processed ${offset} records from table ${tableName}`);
            }
          }

          this.log(`[ClickHouse Sync] Completed syncing table ${tableName}, total records processed: ${offset}`);
        }
      } catch (error) {
