  ...
  try {
    ...
    if (config?.items?.length) {
      config.items = config.items.filter((item) => {
        if (
          !['table', 'field', 'view', 'text', 'number', 'select'].includes(
            item.type
          )
        ) {
          return false;
        ...
        switch (item.type) {
          ...
          case 'field': {
            ...
            if (
              !config.items.find(
                (i: ScriptConfigItem) =>
                  i.type === 'table' && i.key === item.parentTable
              )
            ) {
              return false;
            }
          ...
          case 'select': {
            ...
            if (
              !item?.options?.some((option) => option?.value && option?.label)
            )
              return false;
            break;
          }
...
export const findConfigVariableName = (
  ...
  if (!detectedConfigVar) {
    ...
    if (configStart !== 0 && configEnd !== 0) {
      ...
      if (expressionStart !== 0 && expressionEnd !== 0) {
        ...
        updatedScript =
          scriptContent.slice(0, configStart) + scriptContent.slice(configEnd);
      }
    }
    ...
    if (scriptConfig?.items) {
      ...
      scriptConfig.items.forEach((item: ScriptConfigItem) => {
        if (item.type !== 'field' && item.type !== 'view') {
          if (
            Object.prototype.hasOwnProperty.call(configValues, item.key) &&
            configValues[item.key] != null
          ) {
      ...
      scriptConfig.items.forEach((item: ScriptConfigItem) => {
        if (item.type === 'field' || item.type === 'view') {
          if (
            ...
            const tableId =
              parentTableValue?.value || parentTableValue?.tableId || '';
...
export const validateConfigValues = (
  config: ScriptConfig,
  ...
  config?.items?.forEach((item) => {
    ...
    if (
      (item.type === 'field' || item.type === 'view') &&
      item.parentTable &&
      !tableValues.has(item.parentTable)
    ) {
      errors.push(
        `${item.label || item.key} requires ${item.parentTable} to be selected`
      );
    }
  });
