...
export const excludedFilterColUidt = [
  UITypes.QrCode,
  UITypes.Barcode,
  UITypes.Button,
];
...
export const isFilterDraft = (filter: Filter, col: ColumnTypeForFilter) => {
  ...
  if (
    ...
    comparisonSubOpList(
      filter.comparison_op,
      parseProp(col?.meta)?.date_format
    ).find((compOp) => compOp.value === filter.comparison_sub_op)?.ignoreVal
  ) {
    return false;
  }
  ...
  if (
    comparisonOpList(
      (col.filterUidt ?? col.uidt) as UITypes,
      parseProp(col?.meta)?.date_format
    ).find((compOp) => compOp.value === filter.comparison_op)?.ignoreVal
  ) {
    return false;
  }
...
export const isDynamicFilterAllowed = (
  filter: ColumnFilterType,
  column?: ColumnType,
  ...
  if (!['integer', 'float', 'text', 'string'].includes(abstractType))
    return false;

  return (
    !filter.comparison_op ||
    ['eq', 'lt', 'gt', 'lte', 'gte', 'like', 'nlike', 'neq'].includes(
      filter.comparison_op
    )
  );
};
...
export const getDynamicColumns = (
  ...
  return metaColumns.filter((c: ColumnType) => {
    if (
      excludedFilterColUidt.includes(c.uidt as UITypes) ||
      isVirtualCol(c) ||
    ...
    if (
      [dynamicColAbstractType, filterColAbstractType].every((type) =>
...
export const composeColumnsForFilter = async ({
  ...
  for (const column of rootMeta.columns!) {
    ...
    while (nextCol && nextCol.uidt === UITypes.Lookup) {
      ...
      const lookupRelation: ColumnType | undefined = (
        await getMeta(nextCol.fk_model_id!)
      )?.columns?.find(
        (c) =>
          c.id === (nextCol!.colOptions as LookupType).fk_relation_column_id
      );
      ...
      const relatedTableMeta: TableType | null = await getMeta(
        (lookupRelation?.colOptions as LinkToAnotherRecordType)
          .fk_related_model_id!
      );
      nextCol = relatedTableMeta?.columns?.find(
        (c) => c.id === (nextCol!.colOptions as LookupType).fk_lookup_column_id
      );
...
export const adjustFilterWhenColumnChange = ({
  ...
  filter.comparison_op = comparisonOpList(
    evalUidt,
    parseProp(column.meta)?.date_format
  ).find((compOp) =>
