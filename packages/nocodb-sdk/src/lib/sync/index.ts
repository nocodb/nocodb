...
/**
 * Titles of the system metadata columns table-sync adds to every synced table.
 * Local copy of the titles defined by `syncSystemFields` in
 * `@noco-local-integrations/core` — keep the two in sync when adding/removing a
 * field. Lives here (rather than importing the integrations package) so the
 * SDK, backend, and frontend can all reference it. `isHiddenCol` uses this to
 * keep these columns fully hidden on synced tables, matching by title rather
 * than a system/readonly heuristic — so autogenerated columns (links, lookups)
 * created on a synced table stay visible.
 */
export const SYNC_SYSTEM_COLUMN_TITLES: string[] = [
  'RemoteId',
  'RemoteCreatedAt',
  'RemoteUpdatedAt',
  'RemoteDeletedTime',
