...
/**
 * Minimal view-column shape used by the transformation builder.
 * Backend model types (GridViewColumn, GalleryViewColumn, etc.) all satisfy this.
 */
export interface ViewColumnTransformType {
  fk_column_id?: string;
  width?: string | number;
  show?: boolean | number;
...
export const columnBuilder = builderGenerator<ColumnType, FieldV3Type>({
  allowed: [
    'id',
    'title',
    'uidt',
...
export const columnV3ToV2Builder = builderGenerator<FieldV3Type, ColumnType>({
  ...
  transformFn: (data) => {
    ...
    if (data.uidt === UITypes.Checkbox) {
      const { icon } = meta as Record<string, any>;

      if (icon) {
        const iconIdx = checkboxIconList.findIndex((ic) => ic.label === icon);
        if (iconIdx !== -1) {
          const { label: _label, ...rest } = checkboxIconList[iconIdx];
      ...
      if (icon) {
        ...
        if (iconIdx !== -1) {
          const { label: _label, ...rest } = ratingIconList[iconIdx];
          meta.iconIdx = iconIdx;
          meta.icon = rest;
        }
      }
...
export const viewColumnBuilder = builderGenerator<
  ViewColumnTransformType[],
  Partial<ViewColumnTransformType>[]
>({
  allowed: [
    'fk_column_id',
    'width',
