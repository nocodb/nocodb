...
`;
  ...
  for (const type of availableTypes) {
    integrationsEntry += `import ${capitalize(type)}CommonManifest from '~/integrations/${type}/${type}.manifest';\n`;
  }
  ...
  for (const integration of availableIntegrations) {
    if (integration.entry) {
      integrationsEntry += `import ${capitalize(integration.type)}${capitalize(integration.subType)}Entry from '${integration.entry}';\n`;
    }
    if (integration.form) {
      integrationsEntry += `import ${capitalize(integration.type)}${capitalize(integration.subType)}Form from '${integration.form}';\n`;
    }
    if (integration.manifest) {
      integrationsEntry += `import ${capitalize(integration.type)}${capitalize(integration.subType)}Manifest from '${integration.manifest}';\n`;
    }
  }
  ...
  for (const integration of availableIntegrations) {
    ...
    if (integration.entry) {
      integrationsEntry += `    wrapper: ${capitalize(integration.type)}${capitalize(integration.subType)}Entry,\n`;
    }
    if (integration.form) {
      integrationsEntry += `    form: ${capitalize(integration.type)}${capitalize(integration.subType)}Form,\n`;
    }
    if (integration.manifest) {
      integrationsEntry += `    meta: {
      ...${capitalize(integration.type)}CommonManifest,
      ...${capitalize(integration.type)}${capitalize(integration.subType)}Manifest,
    },\n`;
    }
    integrationsEntry += `  },\n`;
  }
