...
import { ConvertDateFormat } from './BaseModelSqlv2/convertDateFormat';
...
class BaseModelSqlv2 implements IBaseModelSqlV2 {
  ...
  public async execAndParse(
    ...
    if (!options.skipDateConversion) {
      const rollupDateColumns: { column: Column; rollupColumn: Column }[] = [];
      for (const rollupColumn of (this.model?.columns ?? []).filter(
        (col) => col.uidt === UITypes.Rollup,
      )) {
        const rollupColOptions = await rollupColumn.getColOptions<RollupColumn>(
          this.context,
        );
        // only max and min rollup that can return datetime
        if (!['max', 'min'].includes(rollupColOptions.rollup_function)) {
          continue;
        }
        const targetColumn = await rollupColOptions.getRollupColumn(
          this.context,
        );
        if (
          targetColumn.uidt === UITypes.DateTime ||
          targetColumn.uidt === UITypes.Date ||
          isCreatedOrLastModifiedTimeCol(targetColumn) ||
          targetColumn.uidt === UITypes.Formula
        ) {
          rollupDateColumns.push({
            column: rollupColumn,
            rollupColumn: targetColumn,
          });
        }
      }
      data = this.convertDateFormat(data, dependencyColumns, rollupDateColumns);
    }
  ...
  public convertDateFormat(
    ...
    return new ConvertDateFormat(this).convertDateFormat(
      data,
      dependencyColumns,
      rollupColumns,
    );
