...
import { dataWrapper, populatePk } from '~/helpers/dbHelpers';
...
export const baseModelInsert = (baseModel: IBaseModelSqlV2) => {
  ...
  async function nested(data, request: NcRequest, _trx = null, param?) {
    // const driver = trx ? trx : await baseModel.dbDriver.transaction();
    try {
      ...
      let { postInsertOps, preInsertOps, postInsertAuditOps } =
        await baseModel.prepareNestedLinkQb({
          nestedCols,
          data,
          insertObj,
          req: request,
      ...
      await baseModel.beforeInsert(insertObj, baseModel.dbDriver, request);

      await baseModel.prepareNocoData(insertObj, true, request, null, {
        ncOrder: null,
        before: param?.before,
        undo: param?.undo,
      });

      await baseModel.runOps(preInsertOps.map((f) => f()));
      ...
      if (ag) {
        ...
        response = await baseModel.readRecord({
        ...
        if (response?.length) {
          ...
          rowId = await baseModel.execAndParse(query, null, {
            raw: true,
          });
      ...
      rowId = baseModel.extractCompositePK({ ai, ag, rowId, insertObj });
      // when auto generated (with default) pk columns
      if (!rowId && response.length === 1) {
        if (baseModel.model.primaryKeys.length === 1) {
          rowId = response[0][baseModel.model.primaryKeys[0].id];
        } else {
          const responseWithColumnTitle = Object.keys(response[0]).reduce(
            (res, colId) => {
              const col = baseModel.model.columns.find((c) => c.id === colId);
              res[col.title] = response[0][colId];
              return res;
            },
            {},
          );
          rowId = baseModel.extractPksValues(responseWithColumnTitle, true);
        }
      }

      await baseModel.runOps(postInsertOps.map((f) => f(rowId)));
      ...
      if (baseModel.model.primaryKey && rowId !== null && rowId !== undefined) {
        response = await baseModel.readRecord({
          idOrRecord: rowId,
          validateFormula: false,
          ignoreView: true,
