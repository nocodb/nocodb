 ...
 * rather than fall back to record-level `updated_at`, which would surface
 * edits to untracked fields.
 */
export function getLmtSyntheticFormula(
  ...
  const validIds = (trackedIds || []).filter((id) => {
    const tracked = columns.find((c) => c.id === id);
    return tracked && isAllowedLmtTrackedField(tracked);
  });

  if (!metaColumn || !validIds.length) return null;

  return `LAST_MODIFIED_TIME(${validIds.map((id) => `{${id}}`).join(',')})`;
}
