    ...
    try {
      ...
      > | null> => {
        ...
        try {
          ...
          for (const { col, aggregation: agg, resultKey } of aggregateColumns) {
            const aggSql = await applyAggregation({
              baseModelSqlv2: baseModel,
              aggregation: agg,
              column: col,
            });
            if (aggSql) expressions[resultKey] = aggSql;
          }
          ...
          const emptyAggs = JSON.stringify(
            Object.fromEntries(
              aggregateColumns
                .filter(({ resultKey }) => expressions[resultKey])
                .map(({ resultKey, aggregation: agg }) => [
                  resultKey,
                  (
                    [
