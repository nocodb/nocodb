...
export function registerFilterHandlers(svc: FiltersService): void {
  ...
  registerForward(LinkFilterCreateContract, (ctx, p) => svc.linkFilterCreate(ctx, p));
  registerForward(WidgetFilterCreateContract, (ctx, p) => svc.widgetFilterCreate(ctx, p));
  registerForward(RlsPolicyFilterCreateContract, (ctx, p) => svc.rlsPolicyFilterCreate(ctx, p));
  registerForward(RowColorConditionsCreateContract, (ctx, p) => svc.rowColorConditionsCreate(ctx, p));
}
