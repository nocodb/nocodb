...
export const ColumnUpdateContract: OperationContract<typeof columnUpdateSchema> =
  {
    name: 'columnUpdate',
    version: 1,
    entity: MetaTable.COLUMNS,
    ...
    entityTitle: (p) => (p?.column as any)?.title,
    parentId: (p) =>
      (p?.column as any)?.fk_model_id ?? p?.tableId,
    description: (ctx) =>
    ...
    deps: (_p, r) => {
      if (!r || (r as any).uidt !== UITypes.Formula) return [];
      const parsed =
        (r as any).parsed_tree ?? (r as any).colOptions?.parsed_tree;
      return extractFormulaColumnRefs(parsed).map((id) => ({
        entity: MetaTable.COLUMNS,
        id,
      }));
...
export const ColumnDeleteContract: OperationContract<typeof columnDeleteSchema> =
  {
    name: 'columnDelete',
    version: 1,
    entity: MetaTable.COLUMNS,
