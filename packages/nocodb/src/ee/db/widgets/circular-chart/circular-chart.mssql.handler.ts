...
export class CircularChartMssqlHandler extends CircularChartCommonHandler {
  ...
  async getWidgetData(
    ...
    // T-SQL does not accept SELECT-list aliases in GROUP BY (PG/MySQL do).
    // Repeat the underlying expression instead — semantically identical.
    subQuery.groupBy(
      baseModel.dbDriver.raw('??', [categoryColumnNameQuery.builder]),
    );
    ...
    // Cast to NVARCHAR(MAX) — MSSQL's TEXT-equivalent. Preserve original
    // for sorting via original_category.
    ...
    // Final aggregation — final_category is already NVARCHAR(MAX) from the
    // inner CASE; the outer CAST is a no-op kept for shape parity with PG/MySQL.
