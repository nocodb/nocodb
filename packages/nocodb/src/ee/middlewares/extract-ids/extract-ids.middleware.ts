...
import {
  API_TOKEN_ALWAYS_ALLOWED_OPS,
  checkTokenPermission,
} from '~/utils/apiTokenPermissionMap';
...
export class AclMiddleware implements NestInterceptor {
  ...
  async aclFn(
    ...
    */
    ...
    // This gate applies to non-collaborative views. Editors can perform
    // sort/filter/view-column/layout ops on collaborative views (ACL grants
    // it) and on personal views they own. They are blocked on:
    //   - personal views they don't own
    //   - locked views (creator+ only can modify locked views)
    // On collaborative views VIEW_KEY is unset → this check short-circuits
    ...
    // Navigation/discovery ops (workspaceList, baseList, etc.) are always
    // allowed — the service layer filters results by PAT scope.
    if (
      req?.user?.is_api_token &&
      req.user?.api_token_meta?.permissions &&
      !API_TOKEN_ALWAYS_ALLOWED_OPS.has(permissionName)
    ) {
