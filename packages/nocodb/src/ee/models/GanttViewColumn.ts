import GanttViewColumnCE from 'src/models/GanttViewColumn';
...
export default class GanttViewColumn extends GanttViewColumnCE {
  id?: string;
  ...
  constructor(data: GanttViewColumn) {
    super(data);
  ...
  public static async get(
    ...
    let viewColumn =
      ...
      (await NocoCache.get(
        context,
        `${CacheScope.GANTT_VIEW_COLUMN}:${ganttViewColumnId}`,
    ...
    return viewColumn && new GanttViewColumn(viewColumn);
  }

  static async insert(
    context: NcContext,
    column: Partial<GanttViewColumn>,
    ncMeta = Noco.ncMeta,
  ) {
    ...
    if (!insertObj.source_id) {
      const viewRef = await View.get(context, insertObj.fk_view_id, false, ncMeta);
    ...
    return this.get(context, id, ncMeta).then(async (viewColumn) => {
      await NocoCache.appendToList(
        context,
        CacheScope.GANTT_VIEW_COLUMN,
        [column.fk_view_id],
        `${CacheScope.GANTT_VIEW_COLUMN}:${id}`,
      );
      return viewColumn;
    });
  }
  ...
  public static async list(
    ...
    return viewColumns?.map((v) => new GanttViewColumn(v));
  }

  static async update(
    context: NcContext,
    columnId: string,
    body: Partial<GanttViewColumn>,
    ncMeta = Noco.ncMeta,
  ) {
    ...
    await NocoCache.update(
      context,
      `${CacheScope.GANTT_VIEW_COLUMN}:${columnId}`,
      updateObj,
    );
