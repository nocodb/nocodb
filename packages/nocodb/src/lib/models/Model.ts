...
function parseMetaProp(modelorModelList: Model[] | Model) {
  // parse meta property
  for (const model of Array.isArray(modelorModelList) ? modelorModelList : [modelorModelList]) {
    try {
      model.meta = typeof model.meta === 'string' ?  JSON.parse(model.meta):model.meta
    } catch {
      model.meta = {}
    }
  }
...
export default class Model implements TableType {
  ...
  static async updateMeta(
    ...
    if (o) {
      try {
        o.meta = typeof meta === 'string' ? JSON.parse(meta) : meta
        // set cache
        await NocoCache.set(key, o)
      } catch {
      }
