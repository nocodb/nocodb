...
import { Base, Model, SyncIntegration } from '~/models';
...
export default class Integration {
  ...
  constructor(source: Partial<IntegrationType>) {
    Object.assign(this, source);
  }

  protected static castType(source: Integration): Integration {
    return source && new Integration(source);
  }
  ...
  public static async updateBase(
    ...
    const oldIntegration = await Integration.get(context, sourceId, false, ncMeta);

    if (!oldIntegration) NcError.sourceNotFound(sourceId);
    ...
    if (!oldIntegration.order && !updateObj.order) {
      ...
      if (updateObj.order <= 1 && !oldIntegration.isMeta()) {
        updateObj.order = 2;
      }
    }

    // keep order 1 for default source
    if (oldIntegration.isMeta()) {
      updateObj.order = 1;
    }
    ...
    if (!oldIntegration.isMeta()) {
      ...
      if (oldIntegration.order <= 1 && !updateObj.order) {
        updateObj.order = await ncMeta.metaGetNextOrder(MetaTable.BASES, {
          base_id: source.baseId,
        });
      }
    ...
    const returnBase = await this.get(context, oldIntegration.id, false, ncMeta);
  ...
  async delete(
    ...
    const sources = await Integration.list(
      context,
      { baseId: this.base_id },
      ncMeta,
    );
    ...
    const syncIntegrations = await SyncIntegration.list(
      context,
      this.base_id,
      this.id,
      ncMeta,
    );
    for (const syncIntegration of syncIntegrations) {
      await SyncIntegration.delete(context, syncIntegration.id, ncMeta);
    }
  ...
  async softDelete(
    ...
    const sources = await Integration.list(context, { baseId: this.id }, ncMeta);
