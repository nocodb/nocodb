...
export class DuplicateController {
  ...
  @Post([
    '/api/v1/db/meta/duplicate/:baseId/view/:viewId',
    '/api/v2/meta/duplicate/:baseId/view/:viewId',
  ])
  @HttpCode(200)
  @Acl('duplicateView')
  async duplicateView(
    ...
    // load all views
    await model.getViews(context, false);

    const uniqueTitle = generateUniqueName(
      `${model.title} copy`,
      model.views.map((v) => v.title),
    );

    return View.insert(context, {
      copy_from_id: view.id,
      title: uniqueTitle,
      description: view.description,
      ...(view instanceof KanbanView
        ? { fk_grp_col_id: view.fk_grp_col_id }
        : {}),
      ...(view instanceof CalendarView
        ? { calendar_range: view.calendar_range }
        : {}),
    });
  }
