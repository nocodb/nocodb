import {isLinksOrLTAR, isVirtualCol, RelationTypes, UITypes, ViewTypes} from 'nocodb-sdk';
      ...
      for (const col of linkedColumnSet) {
        if (col.colOptions) {
          ...
          if (idMap.has(colOptions.fk_related_model_id)) {
            ...
            } else if (colOptions.type === RelationTypes.HAS_MANY || (colOptions.type === RelationTypes.ONE_TO_ONE && !col.meta?.bt)) {
              // delete col.column_name as it is not required and will cause ajv error (null for LTAR)
              delete col.column_name;
            ...
            } else if (colOptions.type === RelationTypes.BELONGS_TO || (colOptions.type === RelationTypes.ONE_TO_ONE && col.meta?.bt)) {
              if (
                !linkMap.has(
                  `${colOptions.fk_parent_column_id}::${colOptions.fk_child_column_id}`,
                )
          ...
          if (!headers.length) {
            ...
            for (const header of results.data as any) {
              ...
              if (id) {
                ...
                if (col) {
                  if (col.colOptions?.type === RelationTypes.BELONGS_TO || (col.colOptions?.type === RelationTypes.ONE_TO_ONE && col.meta?.bt)) {
