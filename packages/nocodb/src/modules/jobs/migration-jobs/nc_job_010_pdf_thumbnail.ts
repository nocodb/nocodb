...
@Injectable()
export class PdfThumbnailMigration {
  private readonly debugLog = debug('nc:migration-jobs:pdf-thumbnail');
  ...
  log = (...msgs: string[]) => {
    console.log('[nc_job_010_pdf_thumbnail]: ', ...msgs);
  };

  async job() {
    try {
      const isPdfjsInitialized = Noco.isPdfjsInitialized;

      if (!isPdfjsInitialized) {
        this.log(
          `Pdfjs initialization is not supported in this platform at the moment. Skipping thumbnail migration for pdf for now!`,
        );
        return true;
      }
      ...
      const temp_file_references_table = 'nc_temp_pdf_references';
      ...
      const numberOfPdfToBeProcessed = (
        await ncMeta
          .knexConnection(temp_file_references_table)
          .where('thumbnail_generated', false)
          .andWhere('referenced', true)
          .andWhere('mimetype', 'application/pdf')
          .count('*', { as: 'count' })
          .first()
      )?.count;
      ...
      this.log(
        `Starting pdf thumbnail generation for ${numberOfPdfToBeProcessed} pdfs`,
      );
      ...
      const wrapper = async (
        ...
        try {
          ...
          if (thumbnailGenerated.length > 0) {
            ...
            skipPdfs.push(fileReference.id);
          }
        } catch (e) {
          this.log(
            `Error generating thumbnail for file ${fileReference.file_path}`,
          ...
          processedPdfs += 1;
          this.log(
            `Processed ${processedPdfs} out of ${numberOfPdfToBeProcessed} pdfs`,
          );
          const processingPdf = processingPdfs.find(
            (p) => p.id === fileReference.id,
          );

          if (processingPdf) {
            processingPdf.processed = true;
          }
        }
      };
      ...
      while (true) {
        ...
        processingPdfs = processingPdfs.filter((p) => !p.processed);

        const fileReferences = await ncMeta
          .knexConnection(temp_file_references_table)
          .whereNotIn(
            'id',
            skipPdfs.concat(processingPdfs.map((p) => p.id)),
          )
