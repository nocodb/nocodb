...
export class ColumnsService implements IColumnsService {
      ...
      case UITypes.LastModifiedBy:
        {
          ...
          // persist the tracked-field set as junction rows (never in meta)
          if (
            parseProp(colBody.meta)?.fields_mode === 'specific' &&
            Array.isArray(colBody.tracked_field_ids)
          ) {
            await LmtTrackedField.set(
              context,
              savedColumn.id,
    ...
    if (isLinksOrLTAR(param.column) && newColumn) {
      try {
        const relationColOpt =
          await newColumn.getColOptions<LinkToAnotherRecordColumn>(ncMeta);
        if (relationColOpt) {
            ...
            if (relationColOpt.fk_mm_model_id) {
              ...
              if (junction) {
                try {
                  ...
                  await junction.getColumns(ncMeta);
                ...
                await junction.delete(ncMeta, true);
              }
            } else if (
  ...
  async createLTARColumn(
    ...
    if (!isMMLike && (ltarReq.type === 'hm' || ltarReq.type === 'bt')) {
      ...
      const assocCols = await assocModel.getColumns();
  ...
  async getLinkColumnRefTable(
    ...
    const { refContext, mmContext } = colOptions.getRelContext();
