...
export class ColumnsService implements IColumnsService {
  ...
  private async simpleColumnUpdate(
    ...
    if ('title' in param.column) {
      // Only an actual rename is a modification — resubmitting the current
      // title must not trip the guards below.
      if (isColumnPropModified(column, param.column, 'title')) {
  ...
  protected async _runColumnUpdate(
    ...
    // Whether the payload asks to *change* anything beyond description/meta.
    // Diffed against the stored column rather than read off the payload's key
    // set, so submitting a prop with the value the column already holds stays
    // a no-op instead of tripping the guards below.
    const payloadHasNonMetaProps = payloadModifiesColumn(
      column,
      param.column,
      META_ONLY_COLUMN_PROPS,
    );
    ...
    // Simple update path: nothing but title and/or description actually
    // changes — skip heavy logic. Props resubmitted with their stored value
    // change nothing, so they keep a client that echoes back the whole column
    // on this path rather than dropping it into column alteration.
    const isSimpleUpdate =
      !allowUpdateSystemField &&
      Object.keys(param.column).every(
        (k) =>
          SIMPLE_COLUMN_PROPS.has(k) ||
          !isColumnPropModified(column, param.column, k),
      );
    ...
    if (
      ...
      } else if (isAutoGeneratedColumn(column) && colBody.uidt !== column.uidt) {
