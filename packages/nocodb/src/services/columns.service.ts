...
export class ColumnsService implements IColumnsService {
  ...
  private async simpleColumnUpdate(
    ...
    // Mirrors the diff in _runColumnUpdate — a title submitted unchanged must
    // not trip the system-field guard below.
    const titleChanged =
    ...
    // Nothing actually differs (e.g. a title-only PATCH with the current
    // title) — skip the write rather than bumping updated_at and dropping
    // caches for a no-op.
  ...
  protected async _runColumnUpdate(
    ...
    // A property submitted with the value it already has is a no-op, not a
    // modification, so the guards below must not fire on it. Only title and
    // column_name are compared: booleans arrive as 0/1 from sqlite, meta is a
    // JSON string and colOptions is structural, so everything else stays
    // conservatively counted as a change.
