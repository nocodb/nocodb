...
export class ColumnsService implements IColumnsService {
  ...
  protected async _runColumnUpdate(
    ...
    if (
        ...
        if ([UITypes.QrCode, UITypes.Barcode].includes(column.uidt)) {
          ...
          if (
            ...
            validateLmtTrackedFields(context, {
              columnBody: { uidt: column.uidt, meta: colBody.meta },
      ...
      if ('meta' in colBody) {
        validateLmtTrackedFields(context, {
          columnBody: { uidt: colBody.uidt, meta: colBody.meta },
          columns: await table.getColumns(context),
        });
      }

      // allow updating of title and meta only
      await Column.update(context, param.columnId, {
        ...column,
        title: colBody.title,
  ...
  async columnDelete(
    ...
    // prune the deleted column from any LastModifiedTime/LastModifiedBy
    // fields tracking it
    for (const lmtCol of table.columns) {
      if (!isFieldTrackingLmtCol(lmtCol) && !isFieldTrackingLmbCol(lmtCol))
        continue;
      const trackedIds = getLmtTrackedFieldIds(lmtCol);
      if (!trackedIds.includes(column.id)) continue;
      await Column.updateMeta(
        context,
        {
          colId: lmtCol.id,
          meta: {
            ...parseProp(lmtCol.meta),
            tracked_field_ids: trackedIds.filter((id) => id !== column.id),
          },
        },
        ncMeta,
      );
