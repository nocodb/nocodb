...
export class ColumnsService implements IColumnsService {
  ...
  protected async _runColumnUpdate(
    ...
    if (isSyncedColumn) {
      const filtered: Partial<ColumnReqType & { colOptions?: any }> = {};
      if ('meta' in (param.column as any)) {
        (filtered as any).meta = (param.column as any).meta;
      }
      if ('description' in (param.column as any)) {
        (filtered as any).description = (param.column as any).description;
      }
      param.column = filtered as ColumnReqType & { colOptions?: any };
    }
