...
import { AppEvents, UITypes } from 'nocodb-sdk';
import type { FilterReqType, UserType, FilterType, ViewType } from 'nocodb-sdk';
import type { NcContext, NcRequest } from '~/interface/config';
import { AppHooksService } from '~/services/app-hooks/app-hooks.service';
import { FilterOperatorRegistryService } from '~/services/filter-operator-registry.service';
import { validatePayload } from '~/helpers';
import { NcError } from '~/helpers/catchError';
import { Filter, Hook, View, Column } from '~/models';
...
export class FiltersService {
  ...
  private async analyzeViewFilterImpact(
    ...
    try {
      ...
      console.warn(
        `Failed to analyze view ${view.id}: ${error.message}`,
      );
      return null;
    }
  }
