...
export class InviteLinksService {
  ...
  async accept(
    ...
    if (link.email_domain) {
      ...
      // before the account exists; best-effort on CE and on-prem self-signup,
      // which verify nothing. The `email_verified` precondition here was
      // dropped on 2026-09-19 -- nothing in the product ever sets that column,
      // so it refused every redeemer of every domain-restricted link.
