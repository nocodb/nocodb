...
function docRevisionTests() {
  ...
  describe('DocRevision.record', () => {
    it('should INSERT the first revision for a doc', async () => {
      ...
      const rev = await DocRevision.record(ctx, {
        docId: doc.id!,
        version: 1,
      ...
      expect(rev.id).to.be.a('string');
      expect(rev.fk_doc_id).to.equal(doc.id);
      expect(rev.version).to.equal(1);
      expect(rev.title).to.equal('Doc');
      expect(rev.source).to.equal(DocRevisionSource.AUTO);
      expect(rev.created_by).to.equal('user-1');
    ...
    it('should COALESCE same-author writes within the window', async () => {
      ...
      const first = await DocRevision.record(ctx, {
        docId: doc.id!,
        version: 1,
      ...
      const second = await DocRevision.record(ctx, {
        docId: doc.id!,
        version: 2,
      ...
      expect(second.title).to.equal('Doc updated');
      expect(second.content).to.deep.equal(sampleContent('final'));
    ...
    it('should preserve source=restore on the new revision when restoring', async () => {
      ...
      const restored = await DocRevision.record(ctx, {
        docId: doc.id!,
        version: 2,
        content: sampleContent('original'),
        title: 'Doc',
