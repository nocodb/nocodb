...
import 'mocha';
import { ProjectStatus, UITypes } from 'nocodb-sdk';
import { listBase } from '../../factory/base';
import { listRow } from '../../factory/row';
import { getAllTables } from '../../factory/table';
...
function duplicateBaseTests() {
  ...
  it('will duplicate base successfully', async () => {
    ...
    expectedColumns.forEach((colTitle) => {
      expect(columns?.some((col) => col.title === colTitle)).to.eq(
        true,
        `Column ${colTitle} should exist`,
      );
    });
    ...
    expect(row[0].DateTime).to.match(
