...
export default function (API_VERSION: 'v3' | 'v2') {
  ...
  describe(`Base v3`, () => {
    ...
    it.only(`Create base`, async () => {
      ...
      // validate error response
      expect(Object.keys(err.body)).to.include.members(['error', 'message']);
      expect(err.body.error).to.equal('Invalid request body');
      expect(err.body.message).to.equal("Workspace 'invalidId' not found");
    });
    ...
    it(`Update Base v3`, async () => {
      ...
      await request(context.app)
        .patch(`/api/v3/meta/bases/${base.id}`)
    ...
    it(`Delete Base v3`, async () => {
      ...
      await request(context.app)
        .delete(`/api/v3/meta/bases/${base.id}`)
        .set('xc-auth', context.token)
        .send()
        .expect(200);
    ...
    it(`List Base Users v3`, async () => {
      ...
      const response = await request(context.app)
        .get(`/api/v3/meta/bases/${base.id}/users`)
        .set('xc-auth', context.token)
        .send()
        .expect(200);
    ...
    it(`Create, Update and Delete Base Users v3`, async () => {
      const base: Base = await createBase({
        title: 'Test Base',
        description: 'Test Description',
        meta: {
      ...
      await request(context.app)
        .post(`/api/v3/meta/bases/${base.id}/users`)
        .set('xc-auth', context.token)
      ...
      if (isV2) {
        await request(context.app)
          .patch(`/api/v3/meta/bases/${base.id}/users/${updateQueries[0].id}`)
          .set('xc-auth', context.token)
          .send(updateQueries[0])
          .expect(200);
      } else if (isV3) {
        await request(context.app)
          .patch(`/api/v3/meta/bases/${base.id}/users`)
          .set('xc-auth', context.token)
          .send(updateQueries)
          .expect(200);
      }
      ...
      if (isV2) {
        ...
        await request(context.app)
          .delete(`/api/v3/meta/bases/${base.id}/users/${updatedUser.id}`)
          .set('xc-auth', context.token)
          .expect(200);
      } else if (isV3) {
        await request(context.app)
          .delete(`/api/v3/meta/bases/${base.id}/users`)
          .set('xc-auth', context.token)
          .send(deleteQueries)
          .expect(200);
      }
    ...
    async function createBase(baseArgs: CreateBaseArgs) {
      const response = await request(context.app)
        .post(
          isEE
            ? `/api/v3/meta/workspaces/${context.fk_workspace_id}/bases`
            : `/api/v3/meta/bases`,
        )
        .set('xc-auth', context.token)
        .send({
