  ...
  describe(`${apiType.toUpperCase()} api - FORM view (Share)`, () => {
    ...
    const viewTest = (viewType) => {
      it(`Create ${viewType} view`, () => {
        ...
        cy.get(`.nc-view-item.nc-${viewType}-view-item`).contains('Form-1').should('exist')
        ...
        cy.get('.nc-form').find('[placeholder="Form Title"]').clear().type('A B C D')
      ...
      it(`Share form view`, () => {
        cy.get(`.nc-view-item.nc-${viewType}-view-item`).contains('Form-1').click()
        ...
        cy.getActiveModal('.nc-modal-share-view')
          ...
          .then(($obj) => {
            ...
            cy.get('.nc-form-view', { timeout: 10000 }).find('h1').contains('A B C D').should('exist')
