...
// const expectedQrCodeImgSrcValues = [
...
// const expectedQrCodeImgSrcValues = QRCode.toDataURL(value, options)
...
test.describe('Virtual Columns', () => {
  ...
  async function qrCodeVerify(qrColumnTitle: string, expectedQrCodes: string[]) {
    for (let i = 0; i < expectedQrCodes.length; i++) {
      const expectedQrCodeImgSrcValue = await QRCode.toDataURL(expectedQrCodes[i], { width: 150 });
      console.log('FOO expectedQrCodes[i]', expectedQrCodes[i]);
      console.log('FOO expectedQrCodeImgSrcValue', expectedQrCodeImgSrcValue);
      console.log('------------------');
      await dashboard.grid.cell.verifyQrCodeCell({
        index: i,
        columnHeader: qrColumnTitle,
        expectedSrcValue: expectedQrCodeImgSrcValue,
      });
    }
  }
  ...
  test('QrCode', async () => {
    ...
    //   expectedSrcValue: expectedQrCodeImgSrcValues[0],
    // });
    ...
    const expectedQrCodeValues = ['A Corua (La Corua)', 'Abha', 'Abu Dhabi'];
    // for (let i = 1; i < expectedQrCodeValues.length; i++) {
    await qrCodeVerify('QrCode1', expectedQrCodeValues);
    // }
